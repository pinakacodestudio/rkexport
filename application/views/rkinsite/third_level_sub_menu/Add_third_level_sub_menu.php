		      ...
		      <div class="panel panel-default border-panel">
		        <div class="panel-body">
					...
					<form class="form-horizontal" id="formsubmenu">
						<input type="hidden" name="id" value="<?php if(isset($submenurow)){ echo $submenurow['id']; } ?>">
						<div class="form-group" id="submenu_div">
							<label for="submenu" class="col-sm-3 control-label">Select Sub Menu <span class="mandatoryfield">*</span></label>
							<div class="col-sm-6">
								<select id="submenu" class="selectpicker form-control" name="submenuid" data-live-search="true" data-select-on-tab="true" data-size="8">
									...
									<?php foreach($submenudata as $row){ 
                                        if($row['url']==''){?>
										<option value="<?php echo $row['id']; ?>" <?php if(isset($submenurow)){ if($submenurow['submenuid'] == $row['id']){ echo 'selected'; } } ?>><?php echo $row['name']." (".$row['mainmenuname'].")"; ?></option>
									<?php } } ?>
								</select>
							</div>
						</div>
						<div class="form-group" id="menuname_div">
							<label for="name" class="col-sm-3 control-label">Third Level Sub Menu Name <span class="mandatoryfield">*</span></label>
							<div class="col-sm-6">
						...
						<?php 
							...
							if(isset($submenurow)){
								...
								$submenucheckvisible = ($submenurow['thirdlevelsubmenuvisibleinrole']==1)?'checked':'';
								$submenucheckadd = ($submenurow['thirdlevelsubmenuaddinrole']==1)?'checked':'';
								$submenucheckedit = ($submenurow['thirdlevelsubmenueditinrole']==1)?'checked':'';
								$submenucheckdelete = ($submenurow['thirdlevelsubmenudeleteinrole']==1)?'checked':'';
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								...
								<a class="<?=cancellink_class;?>" href="<?=ADMIN_URL?>menu/third-level-sub-menu" title=<?=cancellink_title?>><?=cancellink_text?></a>
							</div>
						</div>
					</form>
				</div>
