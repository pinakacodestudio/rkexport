<?php require_once(APPPATH."views/".ADMINFOLDER.'payment/Paymentheader.php');?>
<div class="row">
                        ...
                        <?php if(!empty($paymentreceiptdata['paymentreceiptstatushistory'])){
                                foreach($paymentreceiptdata['paymentreceiptstatushistory'] as $k=>$prsh){ ?>
                                    <tr>
                                        <td <?=$style?>><?=(++$k)?></td>
                                        <td <?=$style?>>
                                            <?php echo '<span class="label" style="background:#49bf88;">COMPANY</span>'; ?>
                                        </td>
                                        <td <?=$style?>><?=$this->general_model->displaydatetime($prsh['modifieddate'])?></td>
                                        <td><?=($prsh['reason']!="")?ucfirst($prsh['reason']):"-"?></td>
