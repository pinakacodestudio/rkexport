...
if(isset($fedexaccount)){
	...
	define('FEDEX_TRACK_NUMBER', (isset($fedexaccount['trackingcode']))?$fedexaccount['trackingcode']:'');
...
function getProperty($var){
  ...
  if($var == 'key') Return FEDEX_KEY; 
	if($var == 'password') Return FEDEX_PASSWORD; 
	if($var == 'parentkey') Return 'XXX'; 
	if($var == 'parentpassword') Return 'XXX'; 	
	if($var == 'shipaccount') Return FEDEX_ACCOUNT_NUMBER;
	if($var == 'billaccount') Return FEDEX_ACCOUNT_NUMBER;
	...
	if($var == 'meter') Return FEDEX_METER_NUMBER;
  ...
  if($var == 'shipdate') Return '2016-04-21';
	...
	if($var == 'pickuptimestamp'){
		//1540818000 = 06:30 PM
		$pickuptimestamp = (strtotime(date('H:i'))<1540818000)?date("Y-m-d\TH:i:s"):date("Y-m-d\TH:i:s", mktime(8, 0, 0, date("m")  , date("d")+1, date("Y")));
	...
	if($var == 'trackingnumber') Return FEDEX_TRACK_NUMBER;
