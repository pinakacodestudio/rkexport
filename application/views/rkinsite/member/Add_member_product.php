...
<div class="page-content">
    <div class="page-heading">
        <h1><?php if(isset($memberproductdata)){ echo 'Edit'; }else{ echo 'Add'; } ?> <?=$this->session->userdata(base_url().'submenuname')?> Product</h1>
        <small>
            <ol class="breadcrumb">
                <li><a href="<?php echo base_url(); ?><?php echo ADMINFOLDER; ?>dashboard">Dashboard</a></li>
                <li><a href="javascript:void(0)"><?=$this->session->userdata(base_url().'mainmenuname')?></a></li>
                <li><a href="<?php echo base_url().ADMINFOLDER; ?><?=$this->session->userdata(base_url().'submenuurl')?>"><?=$this->session->userdata(base_url().'submenuname')?></a></li>
                <li class="active"><?php if(isset($memberproductdata)){ echo 'Edit'; }else{ echo 'Add'; } ?> <?=$this->session->userdata(base_url().'submenuname')?> Product</li>
            </ol>
        </small>
    </div>
                    ...
                    <div class="panel panel-default border-panel">
                        <div class="panel-body">
                            <div class="col-sm-12 col-md-12 col-lg-12">
                                <?php
                                $membername = '';
                                if($memberdetail['channelid']!=0){
                                    $channellabel="";
                                    $key = array_search($memberdetail['channelid'], array_column($channellist, 'id'));
                                    ...
                                    $membername = $channellabel." ".ucwords($memberdetail['name']);
                                }
                                ?>
                                <div class="col-md-4">
                                    <b><?=Member_label?> Name : </b> <?=$membername?> (<?=$memberdetail['membercode']?>)
                                </div>
                                <div class="col-md-4">
                                    <b>Email : </b> <?=$memberdetail['email']?>
                                </div>
                                <div class="col-md-4">
                                    <b>Mobile No. : </b> <?=$memberdetail['countrycode']!=""?$memberdetail['countrycode']." ".$memberdetail['mobile']:$memberdetail['mobile']?>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-12 col-md-12 col-lg-12">
                                <form class="form-horizontal" id="memberproductform" name="memberproductform">
                                    ...
                                    <?php if(isset($memberproductdata) && !empty($memberproductdata)){ ?>

                                        <input type="hidden" name="productid" value="<?=$productid?>" id="productid">
                                        <input type="hidden" name="priceid" value="<?=$priceid?>" id="priceid">
                                        <input type="hidden" name="memberproductorvariantid" value="<?=$memberproductdata['memberproductorvariantid']?>" id="memberproductorvariantid">
                                        <input type="hidden" name="categoryid" value="<?=$memberproductdata['categoryid']?>" id="categoryid">
                                        <input type="hidden" name="sellermemberid" value="<?=$memberproductdata['sellermemberid']?>" id="sellermemberid">
                                        <div class="form-group row" for="category" id="categoryid_div">
                                            <div class="col-md-6">
                                                <h4><span class="text-muted">Category : </span><?php echo $memberproductdata['categoryname'];?></h4>
                                            </div>
                                            <?php if(REWARDSPOINTS==1) { ?>
                                                <div class="col-md-6 text-right mt-sm mb-sm">
                                                    <span class="label label-info p-xs mr-sm" title="Points for Seller" style="font-size: 12px;">Points for Seller : <?=$memberproductdata['pointsforseller']?></span>
                                                    <span class="label label-info p-xs" title="Points for Buyer" style="font-size: 12px;">Points for Buyer : <?=$memberproductdata['pointsforbuyer']?></span>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php }else{ ?>
                                        <div class="form-group row" for="product" id="sellermemberid_div">
                                            <label class="col-md-4 control-label" for="sellermemberid">Select Seller <span class="mandatoryfield"> * </span></label>
                                            <div class="col-md-4">
                                                <select class="form-control selectpicker" id="sellermemberid" name="sellermemberid" data-live-search="true" data-select-on-tab="true" data-size="5">
                                                    <option value="0">Company</option>
                                                    <?php foreach($memberdata as $member){ ?>
                                                        <option data-code="<?php echo $member['membercode']; ?>" value="<?php echo $member['id']; ?>" <?php if(!empty($orderdata['orderdetail'])){ if($orderdata['orderdetail']['memberid']==$member['id']){ echo "selected"; }} ?>><?php echo ucwords($member['name']); ?></option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                            <div class="col-sm-1 p-n">
                                                <a class="<?=searchsellerbtn_class?>" href="javascript:void(0)" onclick="resetsellerform()" title="<?=searchsellerbtn_title?>" title="Select Seller <?=Member_label?>" data-toggle="modal" data-target="#addsellerModal" style="padding:6px 10px;"><?=searchsellerbtn_text?></a>
                                            </div>
                                        </div>
                                        <div class="form-group row" for="category" id="categoryid_div">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-4">
                                                <select class="form-control selectpicker" id="brandid" name="brandid" data-live-search="true" data-select-on-tab="true" data-size="5">
                                        ...
                                        <div class="form-group row">
                                            <label class="col-md-4 control-label" for="defaultchannelid">Default Channel</label>
                                            <div class="col-md-4">
                                                <select class="form-control selectpicker" id="defaultchannelid" name="defaultchannelid" data-live-search="true" data-select-on-tab="true" data-size="5">
                                                    <?=$channeloption?>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <?php if(isset($memberproductdata) && !empty($memberproductdata)){ ?>
                                            <table class="table table-bordered">
                                                ...
                                                if(!empty($memberproductdata)){
                                                    ...
                                                    if($memberproductdata['pricetype']==0){
                                                        $display_pricedisc = "";
                                                    }else{
                                                        $display_multiprice = "";
                                                    }
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><?=ucwords($memberproductdata['name'])?></td>
                                                        <td class="text-right"><?=number_format($memberproductdata['price'],2,'.',',')?></td>
                                                        <td>
                                                            <div class="col-md-3 pl-n pr-xs" style="<?=$display_pricedisc?>">
                                                                <div class="form-group m-n" id="memberprice0_div">
                                                                    <input type="text" name="memberprice" id="memberprice" class="form-control text-right m-n memberprice" value="<?=((!empty($memberproductdata['multiplepricedata']) && $memberproductdata['pricetype']==0)?number_format($memberproductdata['multiplepricedata'][0]['price'],2,'.',''):"")?>" onkeypress="return decimal_number_validation(event,this.value,8)" placeholder="<?=Member_label?> Price">
                                                                    <input type="hidden" name="singlequantitypricesid" value="<?=($memberproductdata['pricetype']==0 && !empty($memberproductdata['multiplepricedata'])?$memberproductdata['multiplepricedata'][0]['id']:"")?>">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-3 pr-sm pl-xs" style="<?=$display_pricedisc?>">
                                                                <div class="form-group m-n" id="salesprice0_div">
                                                                    <input type="text" name="salesprice" id="salesprice" class="form-control text-right m-n" value="<?=((!empty($memberproductdata['multiplepricedata']) && $memberproductdata['pricetype']==0)?number_format($memberproductdata['multiplepricedata'][0]['salesprice'],2,'.',''):"")?>" onkeypress="return decimal_number_validation(event,this.value,8)" placeholder="Sales Price">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-3 pr-xs pl-n">
                                                                <div class="form-group m-n" id="memberstock_div">
                                                                    <input type="text" name="memberstock" id="memberstock" class="form-control text-right m-n" value="<?=$memberproductdata['memberstock']?>" onkeypress="return decimal_number_validation(event,this.value,8)" placeholder="<?=Member_label?> Stock">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-3 pr-n pl-xs">
                                                                <div class="form-group m-n" for="channelid" id="channelid_div">
                                                                    <select class="form-control selectpicker m-n" id="channelid" name="channelid" data-live-search="true" data-select-on-tab="true" data-size="5">
                                                                        ...
                                                                        <option value="<?php echo $row['id']; ?>" <?php if(isset($memberproductdata)){ if($memberproductdata['channelid']== $row['id']){ echo 'selected'; } } ?>><?php echo $row['name']; ?></option>
                                                                        <?php }?>
                                                                    </select>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-12 p-n">
                                                                <div class="col-md-3 pl-n pr-xs">
                                                                    <div class="form-group m-n p-n" id="minimumsalesprice_div">
                                                                        <div class="col-md-12 p-n text-left">
                                                                            <label for="minimumsalesprice" class="control-label">Min. Sales Price</label>
                                                                            <input type="text" style="width:100%;" name="minimumsalesprice" id="minimumsalesprice" class="form-control text-right" value="<?=$memberproductdata['minimumsalesprice']?>" onkeypress="return decimal_number_validation(event,this.value,10)">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <div class="col-md-2 pl-n pr-xs">
                                                                    <div class="form-group m-n p-n" id="minqty_div">
                                                                        <div class="col-md-12 p-n text-left">
                                                                            ...
                                                                            <input type="text" style="width:100%;" name="minqty" id="minqty" class="form-control text-right" value="<?=($memberproductdata['minimumqty']>0)?$memberproductdata['minimumqty']:""?>" onkeypress="return isNumber(event)" maxlength="4">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <div class="col-md-2 pr-xs pl-xs">
                                                                    <div class="form-group m-n p-n" id="maxqty_div">
                                                                        <div class="col-md-12 p-n text-left">
                                                                            ...
                                                                            <input type="text" style="width:100%;" name="maxqty" id="maxqty" class="form-control text-right" value="<?=($memberproductdata['maximumqty']>0)?$memberproductdata['maximumqty']:""?>" onkeypress="return isNumber(event)" maxlength="4">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                           
                                                                <div class="col-md-2 pl-xs pr-xs" style="<?=$display_pricedisc?>">
                                                                    <div class="form-group m-n p-n" id="discper0_div">
                                                                        <div class="col-md-12 p-n text-left">
                                                                            <label for="discper" class="control-label">Disc. (%)</label>
                                                                            <input type="text" style="width:100%;" name="discper" id="discper" class="form-control text-right discper" value="<?=((!empty($memberproductdata['multiplepricedata']) && $memberproductdata['pricetype']==0 && $memberproductdata['multiplepricedata'][0]['discount']>0)?number_format($memberproductdata['multiplepricedata'][0]['discount'],2,'.',''):"")?>" onkeypress="return decimal_number_validation(event,this.value,5)">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <div class="col-md-3 pr-n pl-xs" style="<?=$display_pricedisc?>">
                                                                    <div class="form-group m-n p-n" id="discamnt0_div">
                                                                        <div class="col-md-12 p-n text-left">
                                                                            ...
                                                                            <input type="text" style="width:100%;" name="discamnt" id="discamnt" class="form-control text-right discamnt" value="<?=((!empty($memberproductdata['multiplepricedata']) && $memberproductdata['pricetype']==0 && $memberproductdata['multiplepricedata'][0]['discount']>0)?number_format(($memberproductdata['multiplepricedata'][0]['price']*$memberproductdata['multiplepricedata'][0]['discount']/100),2,'.',''):"")?>" onkeypress="return decimal_number_validation(event,this.value,10)">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <div class="col-sm-8 pl-n">
                                                                        <div class="col-sm-6 col-xs-6" style="padding-left: 0px;">
                                                                            <div class="radio">
                                                                                <input type="radio" name="pricetype" id="singleqty0" class="pricetype" value="0" <?php if($memberproductdata['pricetype']==0){ echo "checked"; } ?>>
                                                                                <label for="singleqty0">Single Quantity</label>
                                                                            </div>
                                                                        </div>
                                                                        <div class="col-sm-6 col-xs-6 p-n">
                                                                            <div class="radio">
                                                                                <input type="radio" name="pricetype" id="multipleqty0" class="pricetype" value="1" <?php if($memberproductdata['pricetype']==1){ echo "checked"; } ?>>
                                                            ...
                                                            <div class="row" id="multiplepricesection0" style="<?=$display_multiprice?>">
                                                                ...
                                                                <div class="col-md-12">
                                                                    <?php 
                                                                    if(isset($memberproductdata['multiplepricedata']) && $memberproductdata['pricetype']==1){
                                                                        foreach($memberproductdata['multiplepricedata'] as $kp=>$multipleprice){ ?>
                                                                            <div id="countmultipleprice_0_<?=$kp+1?>" class="countmultipleprice0">
                                                                                ...
                                                                                <div class="col-md-2">
                                                                                    <div class="form-group pt-sm mt-n">
                                                                                        <?php if ($kp == 0) { ?>
                                                                                        <?php if (count($memberproductdata['multiplepricedata']) > 1) { ?>
                                                        ...
                                                        <td class="text-center">
                                                            <div class="checkbox"><input id="allowcheck"  type="checkbox" value="1" name="allowcheck" class="checkradios m-n" <?=($memberproductdata['allowproduct']==1)?'checked':''?>>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-12 text-center">
                                            <?php if(isset($memberproductdata)){ ?>
                                            ...
                                            <?php }else{ ?>
                                                ...
                                                <input type="reset" name="reset" value="RESET" class="btn btn-info btn-raised" onclick="resetdata()">
                                            <?php } ?>
                                            <a class="<?=cancellink_class;?>" href="<?=ADMIN_URL?>member/member-detail/<?=$memberid?>/products" title=<?=cancellink_title?>><?=cancellink_text?></a>
                                        </div>
                                    </div>
                                </form>
                            </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Search Seller <?=Member_label?></h4>
                    </div>
