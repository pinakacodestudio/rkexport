						...
						<form class="form-horizontal" id="memberproductform" name="memberproductform">
							...
							<div class="form-group row" for="category" id="categoryid_div">
								...
								<?php if(REWARDSPOINTS==1) { ?>
									<div class="col-md-12 text-left mt-sm mb-sm">
										<span class="label label-info p-xs mr-sm" title="Points for Seller" style="font-size: 12px;">Points for Seller : <?=$productdata['pointsforseller']?></span>
										<span class="label label-info p-xs" title="Points for Buyer" style="font-size: 12px;">Points for Buyer : <?=$productdata['pointsforbuyer']?></span>
									</div>
								<?php } ?>
                          	</div>
							...
							<div id="load_variants row">
								...
								?>
									<div class="load_variantsdiv col-md-6">
										<div class="row variant_div">
											<div class="col-md-4">
												<div class="form-group paddinglr" for="price" id="price_div<?=$pricescnt?>">Price<input type="text" id="price<?=$pricescnt?>" onkeypress="return decimal(event,this.value)" class="form-control prices" disabled="" placeholder="Price" name="price[9]" value="<?=$productcombination[$key][0]['price']?>">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group paddinglr" for="price" id="memberprice_div<?=$pricescnt?>"><?=Member_label?> Price
												<?php
											...
											<div class="col-md-4">
												<div class="form-group paddinglr" for="stock" id="memberstock_div<?=$pricescnt?>"><?=Member_label?> Stock
												<?php
												if($productcombination[$key][0]['membervariantid']!=""){
												?>
												<input type="text" id="memberstock<?=$pricescnt?>" onkeypress="return decimal(event,this.value)" class="form-control stockdivs" placeholder="<?=Member_label?> Stock" name="memberstock[<?=$productcombination[$key][0]['membervariantid']?>]" value="<?=$productcombination[$key][0]['memberstock']?>">
												<?php
												}else{
												?>
												<input type="text" id="memberstock<?=$pricescnt?>" onkeypress="return decimal(event,this.value)" class="form-control stockdivs" placeholder="<?=Member_label?> Stock" name="newmemberstock[<?=$productcombination[$key][0]['priceid']?>]">
												<?php
												}
												?>
								...
								<div class="row">
									<div class="col-md-4">
										<div class="form-group paddinglr" for="universalprice" id="universalprice_div">Price
									...
									<div class="col-md-4">	<div class="form-group paddinglr" for="memberuniversalprice" id="memberuniversalprice_div"><?=Member_label?> Price <input type="text" id="memberuniversalprice" onkeypress="return decimal(event,this.value)" class="form-control prices" placeholder="<?=Member_label?> Price" name="memberuniversalprice" value="<?=$productdata['memberprice'];?>">
										</div>
									</div>
									
									<div class="col-md-4">	
										<div class="form-group paddinglr" for="memberuniversalstock" id="memberuniversalstock_div"><?=Member_label?> Stock <input type="text" id="memberuniversalstock" onkeypress="return decimal(event,this.value)" class="form-control stockdivs" placeholder="<?=Member_label?> Stock" name="memberuniversalstock" value="<?=$productdata['memberstock'];?>">
										</div>
									</div>
							...
							<div class="row">
								...
								<div class="col-sm-8">
								  ...
								  <input type="reset" id="resetbtn" name="reset" value="RESET" class="btn btn-info btn-raised">
									<a class="<?=cancellink_class;?>" href="<?=ADMIN_URL?>member/member-detail/<?=$memberid?>" title=<?=cancellink_title?>><?=cancellink_text?></a>
								</div>
							</div>
						</form>
					</div>
