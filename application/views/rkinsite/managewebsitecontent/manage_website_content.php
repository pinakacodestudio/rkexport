            ...
            <div class="panel panel-default border-panel">
              <div class="panel-heading">
                ...
                <div class="col-md-6 form-group" style="text-align: right;">
                  ...
                  ?>
                  <a class="<?=addbtn_class;?>" href="<?php echo ADMIN_URL; ?>manage_website_content/add-manage-website-content" title=<?=addbtn_title?>><?=addbtn_text;?></a> 
                  <?php
                    }if(strpos($submenuvisibility['submenudelete'],','.$this->session->userdata[base_url().'ADMINUSERTYPE'].',') !== false){
                  ?>
                  <a class="<?=deletebtn_class;?>" href="javascript:void(0)" onclick="checkmultipledelete('','Website Content','<?php echo ADMIN_URL; ?>manage_website_content/deletemulmanagecontent')" title=<?=deletebtn_title?>><?=deletebtn_text;?></a>                
                  <?php } ?>
                </div>
              </div>
                      ...
                      <tr id="tr<?php echo $row['id']; ?>">
                        ...
                        <td>
                          <?php if(strpos($submenuvisibility['submenuedit'],','.$this->session->userdata[base_url().'ADMINUSERTYPE'].',') !== false){ ?>
                            <a class="<?=edit_class;?>" href="<?=ADMIN_URL?>manage_website_content/edit_manage_website_content/<?php echo $row['id']; ?>" title=<?=edit_title?>><?=edit_text;?></a>
                          <?php }if(strpos($submenuvisibility['submenudelete'],','.$this->session->userdata[base_url().'ADMINUSERTYPE'].',') !== false){ ?>
                            <a class="<?=delete_class;?>" href="javascript:void(0)" title=<?=delete_title?> onclick="deleterow(<?=$row['id']; ?>,'','Website Content','<?php echo ADMIN_URL; ?>manage_website_content/deletemulmanagecontent')"><?=delete_text;?></a>
                          <?php } if($row['status']==1){ ?>
                          <span id="span<?=$row['id']; ?>"><a href="javascript:void(0)" onclick="enabledisable(0,<?=$row['id']; ?>,'<?=ADMIN_URL; ?>manage_website_content/manage_website_content_enable_disable','<?=disable_title?>','<?=disable_class?>','<?=enable_class?>','<?=disable_title?>','<?=enable_title?>','<?=disable_text?>','<?=enable_text?>')" class="<?=disable_class?>" title="<?=disable_title?>"><?=stripslashes(disable_text)?></a></span>
                          <?php }else{ ?>
                          <span id="span<?=$row['id']; ?>"><a href="javascript:void(0)" onclick="enabledisable(1,<?=$row['id']; ?>,'<?=ADMIN_URL; ?>manage_website_content/manage_website_content_enable_disable','<?=enable_title?>','<?=disable_class?>','<?=enable_class?>','<?=disable_title?>','<?=enable_title?>','<?=disable_text?>','<?=enable_text?>')" class="<?=enable_class?>" title="<?=enable_title?>"><?=stripslashes(enable_text)?></a></span>
                          <?php } ?>
                        </td> 
                        <td>
                          <div class="checkbox">
