                      ...
                      <div class="col-sm-3">
                        <div class="form-group">
                          <div class="col-sm-12 pl-xs pr-xs">
                              ...
                              <select id="productcategoryid" name="productcategoryid" class="selectpicker form-control" data-live-search="true" data-select-on-tab="true" data-size="5">
                                <option value="0">All Product Category</option>
                                <?php if(!empty($productcategorydata)){ foreach($productcategorydata as $productcategory){ ?>
                                <option value="<?php echo $productcategory['id']; ?>"><?php echo ucwords($productcategory['name']); ?></option>
                                <?php }} ?>
                              </select>
                          </div>
                        </div>
              ...
              <div class="panel-heading">
                ...
                <div class="col-md-6 form-group" style="text-align: right;">
                    <?php if (strpos($submenuvisibility['submenuadd'],','.$this->session->userdata[base_url().'ADMINUSERTYPE'].',') !== false){ ?>
                        <a class="<?=addbtn_class;?>" href="<?=ADMIN_URL?>process-group/add-process-group" title=<?=addbtn_title?>><?=addbtn_text;?></a>
                    <?php } if(strpos($submenuvisibility['submenudelete'],','.$this->session->userdata[base_url().'ADMINUSERTYPE'].',') !== false){ ?>
                        <a class="<?=deletebtn_class;?> delete-btn-align" href="javascript:void(0)" onclick="checkmultipledelete('','Process Group','<?php echo ADMIN_URL; ?>process-group/delete-mul-process-group')" title=<?=deletebtn_title?>><?=deletebtn_text;?></a>
                    <?php } ?>
                </div>
              </div>
              <div class="panel-body no-padding">
                <table id="processgrouptable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                  <thead>
                    <tr>
