...
<body style="background-color: #FFF;">
    <div class="row">
        <div class="col-md-12">
            <p class="text-center" style="font-size: 18px;color: #000"><u><b>Cancelled Orders Report</b></u></p>
        </div>
    </div>
        ...
        <table class="table table-striped table-bordered" cellspacing="0" width="100%">
            <thead>
                <tr>            
                    ...
                    <th class="text-right" <?=$style?>>Total Cancel Orders</th>
                    <?php 
                    if(!empty($year)){ ?>
                        <th <?=$style?>>Year</th>
                    <?php } 
            ...
            <tbody>
                <?php if(!empty($reportdata)){
                    foreach($reportdata as $k=>$row){ ?>
                        <tr>
                            <td <?=$style?>><?=(++$k)?></td>
                            <td <?=$style?>><?=$row->channel?></td>
                            <td <?=$style?>><?=$row->name." (".$row->membercode." - ".$row->mobile.")"?></td>
                            <td class="text-right" <?=$style?>><?=$row->countcanncelorder?></td>
                            <?php
                            if(!empty($year)){ ?>
