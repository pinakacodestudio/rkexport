...
<body style="background-color: #FFF;">
    <div class="row">
        <div class="col-md-12">
            <p class="text-center" style="font-size: 18px;color: #000"><u><b>Purchase Return Report</b></u></p>
        </div>
    </div>
        ...
        <table class="table table-striped table-bordered" cellspacing="0" width="100%">
            <thead>
                <tr>         
                    ...
                    <th class="text-right" <?=$style?>>Total Purchase Return (<?=CURRENCY_CODE?>)</th>   
                ...
                <?php if(!empty($reportdata)){
                    foreach($reportdata as $k=>$row){ ?>
                        <tr>
                            ...
                            <?php if(!empty($row['purchasereturn'])){
                                foreach($row['purchasereturn'] as $i=>$pr){ ?>
                                    <td class="text-right" <?=$style?>><?=numberFormat($pr,2,',')?></td>
                                <?php
                                }
                            }?>
                        </tr>
