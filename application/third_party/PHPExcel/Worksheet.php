...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    */
    ...
    //  Trying To make Custom Function for Any 1 Column Data
    public function SingleColumnDataArray($pColumn = 'A',$minRow='0',$maxRow='10',$returnCellRef=false) {
        $returnValue = array();
        $pcolumn = is_numeric($pColumn)?PHPExcel_Cell::stringFromColumnIndex($pColumn):$pColumn;
        
        $r = -1;
        for ($row = $minRow; $row <= $maxRow; ++$row) {
            $rRef = ($returnCellRef) ? $row : ++$r;
            if ($this->_cellCollection->isDataSet($pcolumn.$row)) {
                $cell = $this->_cellCollection->getCacheData($pcolumn.$row);
                if ($cell->getValue() !== null) {
                    if ($cell->getValue() instanceof PHPExcel_RichText) {
                        $returnValue[$rRef] = $cell->getValue()->getPlainText();
                    } else {
                        $returnValue[$rRef] = $cell->getValue();
                    }
                } else {
                    $returnValue[$rRef] = null;
                }
            } else {
                $returnValue[$rRef] = null;
            }
