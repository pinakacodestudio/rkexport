...
class Cart extends Frontend_Controller {
    ...
    public function index() {
        ...
        if(isset($arrSessionDetails[base_url().'MEMBER_ID'])){
            ...
            // $this->viewData['customerbillingdata'] = $this->Customer->getCustomerDetail($arrSessionDetails[base_url().'MEMBER_ID']);
            // $this->viewData['customershippingdata'] = $this->Customer->getCustomerShippingDetail($arrSessionDetails[base_url().'MEMBER_ID']);
            ...
            if(isset($arrSessionDetails[base_url().'COUPON']) && !empty($arrSessionDetails[base_url().'COUPON'])){
                $this->viewData['coupondata'] = $arrSessionDetails[base_url().'COUPON'];
            }
            if(isset($arrSessionDetails[base_url().'REDEEMPOINT']) && !empty($arrSessionDetails[base_url().'REDEEMPOINT'])){
                $this->viewData['redeempoint'] = $arrSessionDetails[base_url().'REDEEMPOINT'];
                $this->viewData['redeemrate'] = $arrSessionDetails[base_url().'REDEEMRATE'];
            }
            $this->load->model('Member_model', 'Member');
            $this->viewData['redeempoints'] = $this->Member->getCountRewardPoint($arrSessionDetails[base_url().'MEMBER_ID']);
            $this->viewData['channeldata'] = $this->Member->getChannelSettingsByMemberID($arrSessionDetails[base_url().'MEMBER_ID']);
        }
        ...
        $this->frontend_headerlib->add_javascript("cart","cart.js");
        ...
        if(isset($arrSessionDetails[base_url().'PRODUCT']) && !empty($arrSessionDetails[base_url().'PRODUCT'])){
            ...
            if(!empty(json_decode($PostData['cartproduct'],true))){
                ...
                if(empty($cartproduct[0]['coupon'])){
                    $this->session->unset_userdata(base_url().'COUPON');
                }else{
                    // $this->session->set_userdata(array(base_url().'COUPON' => 1));
                }
                for ($i=0; $i < count($product); $i++) {
                    if(isset($cartproduct[$i]['productid']) && $product[$i]['productid']==$cartproduct[$i]['productid'] && $product[$i]['productpriceid']==$cartproduct[$i]['productpriceid']){
                        $productdata[] = array('productid'=>$cartproduct[$i]['productid'],
                                                ...
                                                'referencetype'=>$cartproduct[$i]['referencetype'],
                                                'referenceid'=>$cartproduct[$i]['referenceid']
                                            );
                    }else{
                ...
                $productdata = array(base_url().'PRODUCT' => json_encode($productdata));
            ...
            if(!empty($arrSessionDetails[base_url().'MEMBER_ID'])){
                ...
                for ($i=0; $i < count($product); $i++) {
                    ...
                    if($product[$i]['referencetype']==1){
                        $multipleprice = $this->Product_prices->getProductBasicQuantityPriceDataByPriceID(CUSTOMERCHANNELID,$product[$i]['productpriceid'],$product[$i]['productid']);
            ...
            $this->load->model('Product_model', 'Product');
            $ProductData = $this->Product->getCartProduct();
            // $ProductData = $this->Product->getCartProductBysession($arrSessionDetails);
            // print_r($ProductData);exit;

        }else{
            if(!empty(json_decode($PostData['cartproduct'],true))){
                $productdata = array(base_url().'PRODUCT' => $PostData['cartproduct']);
                $this->session->set_userdata($productdata);
    ...
    public function deletecartproduct(){
        ...
        if(isset($arrSessionDetails[base_url().'PRODUCT']) && !empty($arrSessionDetails[base_url().'PRODUCT'])){
            ...
            if(!empty($arrSessionDetails[base_url().'MEMBER_ID'])){
                $this->load->model('Cart_model', 'Cart');
                $customerid = $arrSessionDetails[base_url().'MEMBER_ID'];

                $this->Cart->Delete(array("memberid"=>$customerid,"priceid"=>$PostData['productpriceid'],"type"=>1));
            }
                ...
                if(DISCOUNTCOUPON==1 && !is_null($memberdata['checkdiscountcoupon']) && $memberdata['checkdiscountcoupon']==1){     
                    ...
                    if(count($vouchercode)>0){
                        if($vouchercode['startdate']>date("Y-m-d") && $vouchercode['startdate']!="0000-00-00"){
                            ...
                            $this->session->unset_userdata(base_url().'COUPON');exit;
                        ...
                        $this->session->set_userdata(array(base_url().'COUPON' => $data));
                    }else{
                        echo json_encode(array("result"=>"fail","data"=>"Coupon code is not valid !"));
                        $this->session->unset_userdata(base_url().'COUPON');
                    }
    ...
    public function add_member_address() {
        ...
        $PostData = $this->input->post();
        $createddate = $this->general_model->getCurrentDateTime();
        $addedby = $this->session->userdata(base_url().'MEMBER_ID');
    ...
    public function update_member_address() {
        ...
        $modifiedby = $this->session->userdata(base_url().'MEMBER_ID');
    ...
    public function payment() {
        ...
        if($paymenttype==1){
            ...
            $PostData['paymentdetail'] = array(
                                        ...
                                        'udf1' => $udf1,
                                        //'udf2' => $udf2,
                                        'surl' => FRONT_URL.'success',
                                        'furl' => FRONT_URL.'failure',
                                    );
            ...
            $Post = array('CUST_ID'=>$memberid,
                            ...
                            //'TXN_AMOUNT'=> '1',
                            'CALLBACK_URL'=>FRONT_URL.'paytm/verifypayment',
                            'EMAIL'=>$PostData['billingemail'],
            ...
            $hash=hash('sha512', $key.'|'.$txnid.'|'.$amount.'|'.$productinfo.'|'.$firstname.'|'.$email.'|||||'.$udf5.'||||||'.$salt);
                ...
                $PostData['paymentdetail'] = array(
                        ...
                        'phone' => $PostData['billingmobileno'],
                        'address1' => $address,
                        'surl' => FRONT_URL.'payubiz/payment',
                        'furl' => FRONT_URL.'failure',
                        'hash' => $hash,
    ...
    public function setredeempointonsession() {
        ...
        if($redeempoint > 0){
            $userdata = array(
                base_url().'REDEEMPOINT' => $redeempoint,
                base_url().'REDEEMRATE' => $conversationrate,
            );
            $this->session->set_userdata($userdata);
        }else{
            $this->session->unset_userdata(base_url().'REDEEMPOINT');
            $this->session->unset_userdata(base_url().'REDEEMRATE');
        }
