...
class OrderPayment extends Admin_Controller {
	...
	function __construct(){
		parent::__construct();
		$this->viewData = $this->getAdminSettings('submenu','OrderPayment');
	...
	public function index(){
        ...
        $this->admin_headerlib->add_javascript("order payment", "pages/order_payment.js");	
    ...
    public function listing() {
		...
		foreach ($list as $index=>$datarow) {
            ...
            if($status == 0){
                ...
                if(strpos($this->viewData['submenuvisibility']['submenudelete'],','.$this->session->userdata[base_url().'ADMINUSERTYPE'].',') !== false){                
                    $Action.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","order payment","'.ADMIN_URL.'payment/delete-mul-payment") >'.delete_text.'</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->Order_Payment->count_all(),
						"recordsFiltered" => $this->Order_Payment->count_filtered(),
						"data" => $data,
				);
	...
	public function payment_edit($id) 
	{
        ...
        $paymentdata = $this->Order_Payment->getPaymentDataById($id);
        ...
        $this->viewData['paymenttransactionsdata'] = $this->Order_Payment->getPaymentTransactions($id);
        $this->admin_headerlib->add_javascript_plugins("bootstrap-datepicker","bootstrap-datepicker/bootstrap-datepicker.js");		
        $this->admin_headerlib->add_javascript("add_payment", "pages/add_order_payment.js");
        $this->load->view(ADMINFOLDER . 'template', $this->viewData);
	}
	public function add_payment()
	{
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $this->Order_Payment->_where = ("paymentreceiptno ='".trim($paymentreceiptno)."'");
            $Count = $this->Order_Payment->CountRecords();
            ...
            if($Count==0){
                ...
                $PaymentReceiptId = $this->Order_Payment->Add($insertdata);
                ...
                if($PaymentReceiptId){   
                    ...
                    if($isagainstreference==1){
                        if(!empty($invoiceidarr)){
                            ...
                            if(!empty($insertData)){
                                $this->Order_Payment->_table = tbl_paymentreceipttransactions;
                                $this->Order_Payment->Add_batch($insertData);
                            }
                        }
                    }
                    ...
                    $this->Order_Payment->_table = tbl_paymentreceiptstatushistory;  
                    $this->Order_Payment->Add($insertstatusdata);

                    $this->Order_Payment->_table = tbl_paymentreceipt;
                    $paymentreceiptno = $this->Order_Payment->generatePaymentReceiptNo();
                    $json = array('error'=>1, "paymentreceiptno"=>$paymentreceiptno, "paymentreceiptid"=>$PaymentReceiptId);

                    if($this->viewData['submenuvisibility']['managelog'] == 1){
                        $this->general_model->addActionLog(1,'Order Payment','Add new payment '.$paymentreceiptno.'.');
                    }
                }else{
                    $json = array('error'=>0);
                }
	...
	public function update_payment()
	{
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $this->Order_Payment->_where = ("id != ".$paymentreceiptid." AND paymentreceiptno ='".trim($paymentreceiptno)."'");
            $Count = $this->Order_Payment->CountRecords();
            ...
            if($Count==0){
				...
				$this->Order_Payment->_where = array("id"=>$paymentreceiptid);
				$Edit = $this->Order_Payment->Edit($updatedata);
                ...
                if($Edit){
                    ...
                    if($isagainstreference==1){
                        if(isset($PostData['removepaymentreceipttransactionsid']) && $PostData['removepaymentreceipttransactionsid']!=''){
                            ...
                            if(!empty($MappingData)){
                                foreach ($MappingData as $row) {
                                    ...
                                    $this->Order_Payment->_table = tbl_paymentreceipttransactions;
                                    $this->Order_Payment->Delete("id=".$row['id']);
                                }
                            }
                        }
                        ...
                        if(!empty($invoiceidarr)){
                            ...
                            if(!empty($delete_arr)){
                                $this->Order_Payment->_table = tbl_paymentreceipttransactions;
                                $this->Order_Payment->Delete(array("id IN (".implode(",",$delete_arr).")"=>null));
                            }
                            if(!empty($insertData)){
                                $this->Order_Payment->_table = tbl_paymentreceipttransactions;
                                $this->Order_Payment->Add_batch($insertData);
                            ...
                            if(!empty($updateData)){
                                $this->Order_Payment->_table = tbl_paymentreceipttransactions;
                                $this->Order_Payment->edit_batch($updateData,"id");
                            }
                        }
                    }else{
                        if(!empty($paymentreceipttransactionsidarr)){
                            $this->Order_Payment->_table = tbl_paymentreceipttransactions;
                            $this->Order_Payment->Delete(array("id IN (".implode(',',array_filter($paymentreceipttransactionsidarr)).")"=>null));
                        }
                    }
                    if($this->viewData['submenuvisibility']['managelog'] == 1){
                        $this->general_model->addActionLog(2,'Order Payment','Edit payment '.$paymentreceiptno.'.');
                    }
    ...
    public function view_payment($id){
        $this->checkAdminAccessModule('submenu','view',$this->viewData['submenuvisibility']);
        $this->viewData['title'] = "View Order Payment";
        $this->viewData['module'] = "orderpayment/View_payment";
        
        $this->viewData['paymentreceiptdata'] = $this->Order_Payment->getPaymentDetails($id);
        ...
        $this->viewData['heading'] = 'Order Payment';
        
        if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(4,'Order Payment','View payment details '.$this->viewData['paymentreceiptdata']['paymentreceiptdetail']['paymentreceiptno'].'.');
        }
    ...
    public function update_status(){
        ...
        $this->Order_Payment->_where = array("id" => $paymentreceiptid);
        $update = $this->Order_Payment->Edit($updateData);
        ...
        if($status!=0){
            $this->Order_Payment->_table = tbl_paymentreceipt;
            $this->Order_Payment->_fields = "sellermemberid,isagainstreference";
            $this->Order_Payment->_where = array("id" => $paymentreceiptid);
            $receiptdata = $this->Order_Payment->getRecordsById();
            
            if(!empty($receiptdata) && $receiptdata['isagainstreference']==1){
                $this->Order_Payment->updatePurchaseInvoiceStatus($receiptdata['sellermemberid'],$paymentreceiptid,$status);
            }
        }
        ...
        if($update) {
            ...
            if($this->viewData['submenuvisibility']['managelog'] == 1){
                $this->Order_Payment->_table = tbl_paymentreceipt;
                $this->Order_Payment->_fields="paymentreceiptno";
                $this->Order_Payment->_where=array("id"=>$paymentreceiptid);
                $paymentdata = $this->Order_Payment->getRecordsByID();

                $this->general_model->addActionLog(2,'Order Payment','Change payment status '.$paymentdata['paymentreceiptno'].'.');
            }
            echo 1;    
        }else{
            echo 0;
    ...
    public function delete_mul_payment()
    {
        ...
        {
            $this->Order_Payment->_table = tbl_paymentreceipttransactions;
            $this->Order_Payment->Delete(array("paymentreceiptid"=>$row));
            ...
            if($this->viewData['submenuvisibility']['managelog'] == 1){
                ...
                $this->Order_Payment->_table = tbl_paymentreceipt;
                $this->Order_Payment->_where = array("id"=>$row);
                $paymentdata = $this->Order_Payment->getRecordsById();
            
                $this->general_model->addActionLog(3,'Order Payment','Delete payment '.$paymentdata['paymentreceiptno'].'.');
            }

            $this->Order_Payment->_table = tbl_paymentreceipt;
            $this->Order_Payment->Delete(array("id"=>$row));
        }
    }	
    public function printPayment(){
        ...
        $PostData = $this->input->post();
        $paymentreceiptid = $PostData['id'];
        $PostData['paymentreceiptdata'] = $this->Order_Payment->getPaymentDetails($paymentreceiptid);

        $this->load->model('Invoice_setting_model','Invoice_setting');
        $PostData['invoicesettingdata'] = $this->Invoice_setting->getShipperDetails();
        
        $PostData['printtype'] = "1";
        $PostData['heading'] = "order payment";

        $html['content'] = $this->load->view(ADMINFOLDER."payment/Printpaymentformat.php",$PostData,true);
        
        if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(0,'Order Payment','Print payment details '.$PostData['paymentreceiptdata']['paymentreceiptdetail']['paymentreceiptno'].'.');
        }
