...
class Purchase_quotation extends Admin_Controller {

    public $viewData = array();
    function __construct(){
        parent::__construct();
        $this->viewData = $this->getAdminSettings('submenu', 'Purchase_quotation');
    ...
    public function index() {
        ...
        if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(4,'Quotation','View purchase quotation.');
        }
        ...
        $this->admin_headerlib->add_javascript("Purchase_quotation", "pages/purchase_quotation.js");
    ...
    public function listing() {
        ...
        $list = $this->Purchase_quotation->get_datatables();
        ...
        foreach ($list as $datarow) {         
            ...
            $quotationstatus = '<div class="dropdown">'.$dropdownmenu.'</div>';
            if(in_array($rollid, $edit) && $status == 0) {
                $actions .= '<a class="'.edit_class.'" href="'.ADMIN_URL.'purchase-quotation/purchase-quotation-edit/'. $datarow->id.'/'.'" title="'.edit_title.'">'.edit_text.'</a>';
            }
            $actions .= '<a href="'.ADMIN_URL.'purchase-quotation/view-purchase-quotation/'. $datarow->id.'/'.'" class="'.view_class.'" title="'.view_title.'" target="_blank">'.view_text.'</a>';                  
            ...
            if($status == 1){
                $actions .= '<a href="'.ADMIN_URL.'purchase-order/purchase-order-add/'.$datarow->id.'" class="btn btn-sm btn-raised btn-primary" title="Add Order"><i class="fa fa-plus"></i></a>';
            }
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Purchase_quotation->count_all(),
                        "recordsFiltered" => $this->Purchase_quotation->count_filtered(),
                        "data" => $data,
                        );
