...
class Import_openingstock extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->viewData = $this->getAdminSettings('submenu', 'Import_openingstock');
    }
    ...
    public function index(){
        ...
        $this->admin_headerlib->add_javascript("Import_openingstock", "pages/import_openingstock.js");
    ...
    public function listing(){
        ...
        $list = $this->Import_openingstock->get_datatables();
        $data = array();
        $counter = $_POST['start'];
        ...
        foreach ($list as $datarow) {
            ...
            if (in_array($rollid, $delete)) {
                $actions.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","Import-openingstock","'.ADMIN_URL.'import-openingstock/delete-mul-Import-openingstock") >'.delete_text.'</a>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Import_openingstock->count_all(),
                        "recordsFiltered" => $this->Import_openingstock->count_filtered(),
                        "data" => $data,
                        );
    ...
    public function importopeningstock(){
        ...
        if ($column0 == "Seller Code *" && $column1 == "SKU *" && $column2 == "Stock *") {
            ...
            if ($totalrows>1) {
                ...
                $this->Product->_where = "productid IN (SELECT id FROM ".tbl_product." WHERE memberid=0 AND channelid=0)";
                ...
                for ($i = 2; $i <= $totalrows; $i++) {
                    ...
                    if ($tr=="") {
                        if (COMPANY_CODE==$sellercode) {
                            $updateData = array("stock"=>$stock);
                            $this->Product->_table = (tbl_productprices);
                            $this->Product->_where = "productid='".$sku."' AND productid IN (SELECT id FROM ".tbl_product." WHERE memberid=0 AND channelid=0)";
                        ...
                        //echo $this->writedb->last_query();//exit;
                            ...
                            //print_r($sellercode);exit;

                            $updateData = array("stock"=>$stock);
                            $this->Product->_table = (tbl_membervariantprices);
                            $this->Product->_where = "memberid='".$sellercode."' AND priceid='".$priceid['id']."'";
