...
class Vehicle_tax extends Admin_Controller {

	public $viewData = array();
	function __construct(){
		parent::__construct();
		$this->viewData = $this->getAdminSettings('submenu','Vehicle_tax');
	...
	public function index() {
		...
		$this->admin_headerlib->add_javascript("vehicle_tax","pages/vehicle_tax.js");
	...
	public function listing() {
        ...
        foreach ($list as $datarow) { 
            ...
            if(in_array($rollid, $edit)) {
                $actions .= '<a class="'.edit_class.'" href="'.ADMIN_URL.'vehicle-tax/vehicle-tax-edit/'. $datarow->id.'/'.'" title="'.edit_title.'">'.edit_text.'</a>';
            }
            //Delete Button
            if(in_array($rollid, $delete)) {
                $actions.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","Vehicletax","'.ADMIN_URL.'vehicle-tax/delete-mul-vehicle-tax") >'.delete_text.'</a>';
	...
	public function vehicle_tax_add() {
		...
		$this->admin_headerlib->add_javascript("add_vehicle_tax","pages/add_vehicle_tax.js");
		$this->load->view(ADMINFOLDER.'template',$this->viewData);
	}
	...
	public function add_vehicle_tax() {
		...
		$this->Vehicle_tax->_where = ("receiptno='".$receiptno."' AND vehicleid=".$vehicleid);
	...
	public function update_vehicle_tax() {
		...
		$this->Vehicle_tax->_where = ("id!=".$vehicletaxid." AND receiptno='".$receiptno."' AND vehicleid=".$vehicleid);
		$Count = $this->Vehicle_tax->CountRecords();
