...
class Sitemap extends Admin_Controller 
{
  ...
  function __construct(){
    parent::__construct();
    $this->viewData = $this->getAdminSettings('submenu','Sitemap');
  ...
  public function index() {
    ...
    if($this->viewData['submenuvisibility']['managelog'] == 1){
      $this->general_model->addActionLog(4,'Sitemap','View sitemap.');
    }
    $this->admin_headerlib->add_javascript("Sitemap","pages/sitemap.js");
  ...
  public function listing() {
    ...
    foreach ($list as $datarow) {
        ...
        if(in_array($rollid, $edit)) {
          $Action .= '<a class="'.edit_class.'" href="'.ADMIN_URL.'sitemap/edit-sitemap/'.$datarow->id.'" title='.edit_title.'>'.edit_text.'</a>';

          if($datarow->status==1){
            $Action .= '<span id="span'.$datarow->id.'"><a href="javascript:void(0)" onclick="enabledisable(0,'.$datarow->id.',\''.ADMIN_URL.'sitemap/sitemap-enable-disable\',\''.disable_title.'\',\''.disable_class.'\',\''.enable_class.'\',\''.disable_title.'\',\''.enable_title.'\',\''.disable_text.'\',\''.enable_text.'\')" class="'.disable_class.'" title="'.disable_title.'">'.stripslashes(disable_text).'</a></span>';
          }else{
              $Action .= '<span id="span'.$datarow->id.'"><a href="javascript:void(0)" onclick="enabledisable(1,'.$datarow->id.',\''.ADMIN_URL.'sitemap/sitemap-enable-disable\',\''.enable_title.'\',\''.disable_class.'\',\''.enable_class.'\',\''.disable_title.'\',\''.enable_title.'\',\''.disable_text.'\',\''.enable_text.'\')" class="'.enable_class.'" title="'.enable_title.'">'.stripslashes(enable_text).'</a></span>';
          }
        }

        if(strpos(trim($this->viewData['submenuvisibility']['submenudelete'],','),$this->session->userdata[base_url().'ADMINUSERTYPE']) !== false){
          $Action.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","Sitemap","'.ADMIN_URL.'sitemap/delete-mul-sitemap") >'.delete_text.'</a>';
        }
        $row[] = $Action;
  ...
  public function add_sitemap() {
    ...
    $this->admin_headerlib->add_javascript("Sitemap","pages/add_sitemap.js");
    $this->load->view(ADMINFOLDER.'template',$this->viewData);
  }
  public function sitemap_add() {
      ...
      $CheckDuplicateValue = $this->Sitemap->CheckDuplicateValue($slug);
      ...
      if ($CheckDuplicateValue != 0){
          ...
          if ($Add) {
            if($this->viewData['submenuvisibility']['managelog'] == 1){
              $this->general_model->addActionLog(1,'Sitemap', "Add new ".$slug." sitemap.");
            }
            echo 1;
          } else {
            echo 0;
  ...
  public function sitemap_update() {
      ...
      $CheckDuplicateValue = $this->Sitemap->CheckDuplicateValue($slug,$Sitemapid);
      if ($CheckDuplicateValue != 0){
        ...
        if($Edit){
          if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(2,'Sitemap', "Edit ".$slug." sitemap.");
          }
          echo 1;
        }else{
          echo 0;
