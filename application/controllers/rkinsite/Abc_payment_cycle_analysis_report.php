...
class Abc_payment_cycle_analysis_report extends Admin_Controller 
{

    public $viewData = array();
    function __construct(){
        parent::__construct();
        $this->viewData = $this->getAdminSettings('submenu', 'Abc_payment_cycle_analysis_report');
    ...
    public function index() {
        ...
        $this->viewData['module'] = "abc_payment_cycle_analysis_report/Abc_payment_cycle_analysis_report";

        $this->viewData['channeldata'] = $this->Abc_payment_cycle_analysis_report->getChannelListOnABCPAymentReport();

        if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(4,'ABC Payment Cycle Analysis Report','View ABC payment cycle analysis report.');
        }
        $this->admin_headerlib->add_javascript_plugins("bootstrap-datepicker","bootstrap-datepicker/bootstrap-datepicker.js");
        $this->admin_headerlib->add_javascript("Abc_payment_cycle_analysis_report","pages/abc_payment_cycle_analysis_report.js");
    ...
    public function listing() {
        ...
        foreach ($list as $datarow) {         
            ...
            $row[] = '<a href="'.ADMIN_URL.'member/member-detail/'.$datarow->id.'" title="'.ucwords($datarow->name).'" target="_blank">'.$channellabel." ".ucwords($datarow->name).' ('.$datarow->membercode.')</a>';
    ...
    public function getMemberOnPaymentCycleReportByChannel(){
        ...
        $memberdata = $this->Abc_payment_cycle_analysis_report->getMemberOnPaymentCycleReportByChannel($PostData['channelid']);
    ...
    public function exportToExcelABCPaymentCycleReport(){
        ...
        if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(0,'ABC Payment Cycle Analysis Report','Export to excel ABC payment cycle analysis report.');
        }
    ...
    function exportToPDFABCPaymentCycleReport() {
        ...
        if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(0,'ABC Payment Cycle Analysis Report','Export to PDF ABC payment cycle analysis report.');
        }
    ...
    public function printabcpaymentcyclereport()
    {
        ...
        if ($this->viewData['submenuvisibility']['managelog'] == 1) {
            $this->general_model->addActionLog(0, 'ABC Payment Cycle Analysis Report', 'Print ABC payment cycle analysis report.');
        }
        $PostData = $this->input->post();
