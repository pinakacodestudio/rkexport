...
class Target_offer extends Admin_Controller {

    public $viewData = array();
    function __construct(){
        parent::__construct();
        $this->viewData = $this->getAdminSettings('submenu','Target_offer');
    ...
    public function index() {
       ...
       /*  echo "<pre>";
        print_r($this->viewData['offerdata']);
        exit; */

        if($this->viewData['submenuvisibility']['managelog'] == 1){
            $this->general_model->addActionLog(4,'Target offer','View target offer.');
        }
        ...
        $this->admin_headerlib->add_javascript("target_offer", "pages/target_offer.js");
    ...
    public function listing() {   
        ...
        foreach ($list as $datarow) {         
            ...
            if($datarow->channelid != 0){
                ...
                $membername = $channellabel."<a href='".ADMIN_URL."member/member-detail/".$datarow->memberid."' target='_blank'>".$datarow->membername." (".$datarow->membercode.")"."</a>";
            }else{
                $membername = '-';
            }
            ...
            $row[] = ++$counter;
            $row[] = "<a href='".ADMIN_URL."offer/view-offer/".$datarow->id."' target='_blank'>".ucwords($datarow->name)."</a>";
            $row[] = $membername;
            $row[] = $sellername;
            ...
            if($datarow->sellermemberid == 0){
                if($datarow->countgiftproduct == 0){
                    if($datarow->iscn==1){
                        ...
                        $actions .= '<a href="'.ADMIN_URL.'credit-note/credit-note-add/'.$datarow->memberid.'/'.$datarow->id.'" class="btn btn-info btn-xs">Create Credit Note</a>';
                    }
                }else{
