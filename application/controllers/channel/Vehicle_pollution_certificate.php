...
class Vehicle_pollution_certificate extends Channel_Controller {

	public $viewData = array();
	function __construct(){
		parent::__construct();
		$this->viewData = $this->getChannelSettings('submenu','Vehicle_pollution_certificate');
	...
	public function index() {
		...
		$this->channel_headerlib->add_javascript("vehicle_pollution_certificate","pages/vehicle_pollution_certificate.js");
	...
	public function listing() {
        ...
        foreach ($list as $datarow) { 
            ...
            if(in_array($rollid, $edit)) {
                $actions .= '<a class="'.edit_class.'" href="'.CHANNEL_URL.'vehicle-pollution-certificate/vehicle-pollution-certificate-edit/'. $datarow->id.'/'.'" title="'.edit_title.'">'.edit_text.'</a>';
            }
            //Delete Button
            if(in_array($rollid, $delete)) {
                $actions.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","vehiclepollutioncertificate","'.CHANNEL_URL.'vehicle-pollution-certificate/delete-mul-Vehicle-pollution-certificate") >'.delete_text.'</a>';
	...
	public function vehicle_pollution_certificate_add() {
		...
		$this->channel_headerlib->add_javascript("add_vehicle_pollution_certificate","pages/add_vehicle_pollution_certificate.js");
		$this->load->view(CHANNELFOLDER.'template',$this->viewData);
	}
	...
	public function add_vehicle_pollution_certificate() {
		...
		$this->Vehicle_pollution_certificate->_where = ("pcno='".$pcno."' AND issuingauthority='".$issuingauthority."' AND vehicleid='".$vehicleid."' AND channelid='".$channelid."' AND memberid='".$memberid."' ");
	...
	public function update_vehicle_pollution_certificate() {
        ...
        $this->Vehicle_pollution_certificate->_where = ("id!='".$pollutioncertificateid."' AND pcno='".$pcno."' AND issuingauthority='".$issuingauthority."' AND vehicleid='".$vehicleid."' AND channelid='".$channelid."' AND memberid='".$memberid."' ");
