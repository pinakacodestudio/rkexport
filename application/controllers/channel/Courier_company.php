...
class Courier_company extends Channel_Controller {

    public $viewData = array();
    function __construct(){
        parent::__construct();
        $this->viewData = $this->getChannelSettings('submenu', 'Courier_company');
    ...
    public function index() {
        ...
        $this->channel_headerlib->add_javascript("Courier_company", "pages/courier_company.js");
		...
		foreach ($list as $datarow) {
            ...
            if($datarow->type==1 && $MEMBERID==$datarow->memberid){
                ...
                if(strpos($this->viewData['submenuvisibility']['submenuedit'],','.$this->session->userdata[CHANNEL_URL.'ADMINUSERTYPE'].',') !== false){
                    $Action .= '<a class="'.edit_class.'" href="'.CHANNEL_URL.'courier-company/courier-company-edit/'.$datarow->id.'" title='.edit_title.'>'.edit_text.'</a>';
                }
                ...
                if(strpos($this->viewData['submenuvisibility']['submenudelete'],','.$this->session->userdata[CHANNEL_URL.'ADMINUSERTYPE'].',') !== false){                
                    $Action.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","Courier&nbsp;Company","'.CHANNEL_URL.'courier-company/delete-mul-courier-company") >'.delete_text.'</a>';
    ...
    public function courier_company_add() {
        ...
        $this->channel_headerlib->add_javascript("Courier_company", "pages/add_courier_company.js");
        $this->load->view(CHANNELFOLDER . 'template', $this->viewData);
    }
    ...
    public function courier_company_edit($id) {
        $this->checkAdminAccessModule('submenu','edit',$this->viewData['submenuvisibility']);
        $this->viewData['title'] = "Edit Courier Company";
        $this->viewData['module'] = "courier_company/Add_courier_company";
        $this->viewData['action'] = "1"; //Edit

        $this->Courier_company->_where = array('id' => $id);
