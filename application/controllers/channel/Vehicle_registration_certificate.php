...
class Vehicle_registration_certificate extends Channel_Controller {

	public $viewData = array();
	function __construct(){
		parent::__construct();
		$this->viewData = $this->getChannelSettings('submenu','Vehicle_registration_certificate');
	...
	public function index() {
		...
		$this->channel_headerlib->add_javascript("vehicle_registration_certificate","pages/vehicle_registration_certificate.js");
	...
	public function listing() {
        ...
        foreach ($list as $datarow) { 
            ...
            if(in_array($rollid, $edit)) {
                $actions .= '<a class="'.edit_class.'" href="'.CHANNEL_URL.'vehicle-registration-certificate/vehicle-registration-certificate-edit/'. $datarow->id.'/'.'" title="'.edit_title.'">'.edit_text.'</a>';
            }
            //Delete Button
            if(in_array($rollid, $delete)) {
                $actions.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","Vehicleregistrationcertificate","'.CHANNEL_URL.'vehicle-registration-certificate/delete-mul-Vehicle-registration-certificate") >'.delete_text.'</a>';
	...
	public function vehicle_registration_certificate_add() {
		...
		$this->channel_headerlib->add_javascript("add_vehicle_registration_certificate","pages/add_vehicle_registration_certificate.js");
		$this->load->view(CHANNELFOLDER.'template',$this->viewData);
	}
	...
	public function add_vehicle_registration_certificate() {
		...
		$this->Vehicle_registration_certificate->_where = ("rcno='".$rcno."' AND vehicleid='".$vehicleid."' AND channelid='".$channelid."' AND memberid='".$memberid."' ");
	...
	public function update_vehicle_registration_certificate() {
		...
		$this->Vehicle_registration_certificate->_where = ("id!=".$registrationcertificateid." AND rcno='".$rcno."' AND vehicleid='".$vehicleid."' AND channelid='".$channelid."' AND memberid='".$memberid."'");
		$Count = $this->Vehicle_registration_certificate->CountRecords();
