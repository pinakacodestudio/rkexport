...
class Product_review extends Channel_Controller {

    public $viewData = array();

    function __construct() {
        parent::__construct();
        $this->viewData = $this->getChannelSettings('submenu', 'Product_review');
    ...
    public function index() {
        ...
        $this->channel_headerlib->add_bottom_javascripts("Product_review", "pages/product_review.js");
    ...
    public function listing() {
        ...
        $list = $this->Product_review->get_datatables($MEMBERID,$CHANNELID);
        /* echo "<pre>";
        print_r($list);
        exit; */
        $data = array();
        ...
        foreach ($list as $Productreview) {
            ...
            if($Productreview->memberid!=0){
                $row[] = $channellabel.'<a href="'.CHANNEL_URL.'member/member-detail/'.$Productreview->memberid.'" target="_blank" title="'.ucwords($Productreview->membername).'">'.ucwords($Productreview->membername).' ('.$Productreview->membercode.')</a>';    
            }else{
            ...
            $row[] = '<a href="'.CHANNEL_URL.'product/view-product/'.$Productreview->productid.'" target="_blank" title="'.ucwords($Productreview->productname).'">'.ucwords($Productreview->productname).'</a>';
            ...
            if(strpos($this->viewData['submenuvisibility']['submenudelete'],','.$this->session->userdata[CHANNEL_URL.'ADMINUSERTYPE'].',') !== false){
                $Action .= '<a href="javascript:void(0)" onclick="deleterow('.$Productreview->id.',\'\',\'Product Review\',\''.CHANNEL_URL.'product-review/delete_mul_product_review\')" class="'.delete_class.'" title="'.delete_title.'">'.stripslashes(delete_text).'</a>';
            }
        ...
        $output = array(
                        ...
                        "recordsFiltered" => $this->Product_review->count_filtered($MEMBERID,$CHANNELID),
                        "data" => $data,
                );
    ...
    public function productreviewapproveunapprove() {
        ...
        $MEMBERID = $this->session->userdata(base_url().'MEMBERID');
        $updatedata = array("status" => $PostData['val'], "modifieddate" => $modifieddate,"modifiedby"=>$MEMBERID);
    ...
    public function changereviewtype() {
        ...
        $MEMBERID = $this->session->userdata(base_url().'MEMBERID');
        $modifieddate = $this->general_model->getCurrentDateTime();
        $updatedata = array("type" => $PostData['type'], "modifieddate" => $modifieddate,"modifiedby"=>$MEMBERID);
