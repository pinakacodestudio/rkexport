...
class Stock_general_voucher extends Channel_Controller {

    public $viewData = array();
    function __construct(){
        parent::__construct();
        $this->viewData = $this->getChannelSettings('submenu', 'Stock_general_voucher');
    ...
    public function index() {
        ...
        $this->channel_headerlib->add_javascript("stock_general_voucher", "pages/stock_general_voucher.js");
    ...
    public function listing() { 
        ...
        foreach ($list as $datarow) {         
            ...
            if(in_array($rollid, $edit)) {
                $actions .= '<a class="'.edit_class.'" href="'.CHANNEL_URL.'stock-general-voucher/stock-general-voucher-edit/'. $datarow->id.'/'.'" title="'.edit_title.'">'.edit_text.'</a>';
            }
            if(in_array($rollid, $delete)) {
                $actions.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->voucherproductsid.',"","Stock&nbsp;General&nbsp;Voucher","'.CHANNEL_URL.'stock-general-voucher/delete-mul-stock-general-voucher") >'.delete_text.'</a>';
            ...
            $row[] = '<a href="'.CHANNEL_URL.'product/view-product/'.$datarow->productid.'" target="_blank">'.$datarow->productname.'</a>';
    ...
    public function add_stock_general_voucher() {
        ...
        $this->viewData['narrationdata'] = $this->Narration->getActiveNarration($CHANNELID,$MEMBERID);

        $this->viewData['voucherno'] = $this->general_model->generateTransactionPrefixByType(4,$CHANNELID,$MEMBERID);
        ...
        $this->channel_headerlib->add_javascript_plugins("bootstrap-datepicker","bootstrap-datepicker/bootstrap-datepicker.js");
        $this->channel_headerlib->add_javascript("add_stock_general_voucher", "pages/add_stock_general_voucher.js");
        $this->load->view(CHANNELFOLDER.'template',$this->viewData);
    }
    public function stock_general_voucher_add() {
        ...
        if($Count==0){
            ...
            if($VoucherID){
                $this->general_model->updateTransactionPrefixLastNoByType(4,$CHANNELID,$MEMBERID);
    ...
    public function update_stock_general_voucher() {
        ...
        if($Count==0){
            ...
            if($isUpdated){
                ...
                if(!empty($productidArr)){
                    $insertData = $updateData = $isUpdateData = array();
                    foreach($productidArr as $i=>$productid){
