...
class Offer extends Channel_Controller {

    public $viewData = array();
    function __construct(){
        parent::__construct();
        $this->viewData = $this->getChannelSettings('submenu', 'Offer');
    ...
    public function index() {
        ...
        $this->channel_headerlib->add_javascript("Offer", "pages/offer.js");
     ...
     public function listing() {   
        ...
        foreach ($list as $datarow) {         
            $row = array();
            $actions = '';
            $checkbox = $channelname = $activatestatus = '';
            $offerstatus = $datarow->offerstatus;
            if($datarow->usertype==1 && $datarow->addedby==$MEMBERID){
                if(in_array($rollid, $edit)) {
                    $actions .= '<a class="'.edit_class.'" href="'.CHANNEL_URL.'offer/edit-offer/'. $datarow->id.'/'.'" title="'.edit_title.'">'.edit_text.'</a>';
                }
                if(strpos($this->viewData['submenuvisibility']['submenuedit'],','.$this->session->userdata[CHANNEL_URL.'ADMINUSERTYPE'].',') !== false){

                    if($datarow->status==1){
                        $actions .= '<span id="span'.$datarow->id.'"><a href="javascript:void(0)" onclick="enabledisable(0,'.$datarow->id.',\''.CHANNEL_URL.'offer/offer-enable-disable\',\''.disable_title.'\',\''.disable_class.'\',\''.enable_class.'\',\''.disable_title.'\',\''.enable_title.'\',\''.disable_text.'\',\''.enable_text.'\')" class="'.disable_class.'" title="'.disable_title.'">'.stripslashes(disable_text).'</a></span>';
                    }
                    else{
                        $actions .='<span id="span'.$datarow->id.'"><a href="javascript:void(0)" onclick="enabledisable(1,'.$datarow->id.',\''.CHANNEL_URL.'offer/offer-enable-disable\',\''.enable_title.'\',\''.disable_class.'\',\''.enable_class.'\',\''.disable_title.'\',\''.enable_title.'\',\''.disable_text.'\',\''.enable_text.'\')" class="'.enable_class.'" title="'.enable_title.'">'.stripslashes(enable_text).'</a></span>';
                    }
                    
                }
                if(in_array($rollid, $delete)) {
                    $actions.='<a class="'.delete_class.'" href="javascript:void(0)" title="'.delete_title.'" onclick=deleterow('.$datarow->id.',"","Offer","'.CHANNEL_URL.'offer/delete-mul-offer") >'.delete_text.'</a>';
            ...
            $actions .= '<a href="'.CHANNEL_URL.'offer/view-offer/'. $datarow->id.'/'.'" class="'.view_class.'" title="'.view_title.'" target="_blank">'.view_text.'</a>';
            if(($datarow->usertype==1 && $datarow->addedby==$MEMBERID) || $datarow->type==2 || $datarow->type==3){
                $activatestatus = '';
            }else{
                $currentdate = $this->general_model->getCurrentDate();
                $isvalid = 0;
                if($datarow->startdate!="0000-00-00" && $datarow->startdate<=$currentdate && $datarow->enddate!="0000-00-00" && $datarow->enddate>=$currentdate){
                    $isvalid = 1;
                }
                if(($datarow->startdate=="0000-00-00" && $datarow->enddate=="0000-00-00" && $datarow->createddate<=$currentdate) || $isvalid==1){
                    if($offerstatus == 0){
                        $dropdownmenu = '<button class="btn btn-danger '.STATUS_DROPDOWN_BTN.' btn-raised dropdown-toggle" data-toggle="dropdown" id="btndropdown'.$datarow->id.'">Deactivate <span class="caret"></span></button>
                                    <ul class="dropdown-menu" role="menu">
                                            <li id="dropdown-menu">
                                            <a onclick="changeofferstatus(1,'.$datarow->id.')">Activate</a>
                                            </li>
                                        </ul>';
                    }else if($offerstatus == 1){
                        $dropdownmenu = '<button class="btn btn-success '.STATUS_DROPDOWN_BTN.' btn-raised dropdown-toggle" data-toggle="dropdown" id="btndropdown'.$datarow->id.'">Activate <span class="caret"></span></button><ul class="dropdown-menu" role="menu">
                                        <li id="dropdown-menu">
                                        <a onclick="changeofferstatus(0,'.$datarow->id.')">Deactivate</a>
                                            </li>
                                        </ul>';
                    }
                    $activatestatus = '<div class="dropdown" style="float: left;">'.$dropdownmenu.'</div>';
                }
            }
    ...
    public function offer_add() {
        ...
        $this->viewData['purchaseproductdata'] = $this->Product->getProductByCategoryId($MEMBERID,0,1);
        $this->viewData['offerproductdata'] = $this->Product->getProductByCategoryId($MEMBERID);
        ...
        $this->channel_headerlib->add_javascript("Offer", "pages/add_offer.js");
        $this->load->view(CHANNELFOLDER.'template',$this->viewData);
    }
    ...
    public function update_offer()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($Count)){     
                ...
                $OfferId = $this->Offer->Edit($updateData);
                ...
                if($OfferId){
                    ...
                    echo json_encode(array("error"=>"0")); // Offer not inserted 
                }
            } else {
    ...
    public function view_offer($offerid)
    {
        ...
        $this->viewData['myoffer'] = ($this->viewData['offerdata']['offerdata']['usertype']==1 && $this->viewData['offerdata']['offerdata']['addedby']==$MEMBERID)?1:0;
        ...
        $this->channel_headerlib->add_javascript("view_offer", "pages/view_offer.js");
        $this->load->view(CHANNELFOLDER.'template',$this->viewData);
    }
    ...
    public function delete_mul_offer() {
        ...
        foreach ($ids as $row) {
            ...
            $this->Offer->_table = tbl_offerimage;
            $this->Offer->_fields = 'id,filename';
            $this->Offer->_where = array('offerid'=>$row);
            $imagesdata = $this->Offer->getRecordsByID();
            if(!empty($imagesdata)){
    ...
    public function changeofferstatus(){
        ...
        $this->Offer->_where = ("offerid=".$offerid." AND memberid=".$MEMBERID);
        $offerdata = $this->Offer->getRecordsById();

        if(!empty($offerdata)){
            if($status==1){
                $updateData = array("status"=>$status,"createddate"=>$modifieddate,"addedby"=>$MEMBERID);
            }else{
                $updateData = array("status"=>$status,"modifieddate"=>$modifieddate,"modifiedby"=>$MEMBERID);
            }
    ...
    public function offerorderslisting() {
        ...
        foreach ($list as $datarow) {
            ...
            if($MEMBERID == $datarow->memberid){
                ...
                $row[] = '<a href="'.CHANNEL_URL.'member/member-detail/'.$datarow->memberid.'" title="'.ucwords($datarow->membername).'" target="_blank">'.$channellabel." ".ucwords($datarow->membername).' ('.$datarow->membercode.')</a>';
            }
            ...
            $row[] = '<a href="'.CHANNEL_URL.'purchase-4order/view-order/'.$datarow->id.'" title="'.viewpdf_title.'" target="_blank">'.$datarow->orderid.'</a>';
