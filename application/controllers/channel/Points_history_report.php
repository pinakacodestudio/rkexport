...
class Points_history_report extends Channel_Controller {
    ...
    function __construct(){
        ...
        $this->viewData = $this->getChannelSettings('submenu', 'Points_history_report');
    }
    public function index() {
        ...
        $this->viewData['rewardpoint'] = (!empty($countrewards))?$countrewards['rewardpoint']:0;

        $this->channel_headerlib->add_javascript_plugins("bootstrap-datepicker","bootstrap-datepicker/bootstrap-datepicker.js");
        $this->channel_headerlib->add_javascript("Points_history_report", "pages/points_history_report.js");
        ...
        foreach ($list as $datarow) {         
            ...
            if($datarow->sellerchannelid != 0){
                ...
                if($this->session->userdata(base_url().'MEMBERID') == $datarow->sellerid){
                    $row[] = $channellabel.$datarow->sellername.' ('.$datarow->sellercode.')';
                }else{
                    $row[] = $channellabel.'<a href="'.CHANNEL_URL.'member/member-detail/'.$datarow->sellerid.'" target="_blank" title="'.$datarow->sellername.'">'.$datarow->sellername.' ('.$datarow->sellercode.')'."</a>";
                }
            }else{
            ...
            if($datarow->buyerchannelid != 0){
                ...
                if($this->session->userdata(base_url().'MEMBERID') == $datarow->buyerid){
                    $row[] = $channellabel.$datarow->buyername.' ('.$datarow->buyercode.')';
                }else{
                    $row[] = $channellabel.'<a href="'.CHANNEL_URL.'member/member-detail/'.$datarow->buyerid.'" target="_blank" title="'.$datarow->buyername.'">'.$datarow->buyername.' ('.$datarow->buyercode.')'."</a>";
                }
            }else{
                $row[] = '<span class="label" style="background:#49bf88;">COMPANY</span>';
            }
           ...
           if(!empty($datarow->orderid)){
            $detail = $datarow->detail.'<br><br><b>Order ID: </b><a href="'.CHANNEL_URL.'order/view-order/'.$datarow->orderid.'" target="_blank" title="'.$datarow->ordernumber.'">'.$datarow->ordernumber."</a>";
           }else{
