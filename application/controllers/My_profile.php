...
class My_profile extends Frontend_Controller {
    ...
    public function index() {
        ...
        $memberid = $this->session->userdata(base_url().'MEMBER_ID');
        ...
        $this->viewData['recentorder'] = $this->Order->getOrdersOnFront($memberid,10);
        
        $this->viewData['orderdata'] = $this->Order->getOrdersOnFront($memberid);
        $this->viewData['userdata'] = $this->Member->getMemberDataByIDForEdit($memberid);
        ...
        // echo "<pre>"; print_r($this->viewData['userdata']); exit;
        ...
        $this->frontend_headerlib->add_plugin("dataTables.bootstrap", "datatables/dataTables.bootstrap.css");
        $this->frontend_headerlib->add_javascript_plugins("jquery.dataTables", "datatables/jquery.dataTables.js");
        $this->frontend_headerlib->add_javascript_plugins("dataTables.bootstrap", "datatables/dataTables.bootstrap.js");

        $this->frontend_headerlib->add_stylesheet("bootstrap-imageupload", "bootstrap-imageupload.css");
        $this->frontend_headerlib->add_javascript("bootstrap-imageupload-js", "bootstrap-imageupload.js");
		...
		$this->frontend_headerlib->add_javascript("my_profile","my_profile.js");
		...
		if (empty($Check)) {
			...
			if(empty($Checkemail)){            
				...
				$userdata = array(
					...
					base_url().'MEMBER_EMAIL' => $email,
					base_url().'MEMBER_PROFILE_IMAGE' => $image
				);
