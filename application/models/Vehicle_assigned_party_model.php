...
class Vehicle_assigned_party_model extends Common_model {
	...
	public $column_order = array('av.date','partyname','ct.name','p.name');
    public $column_search = array("(CONCAT(pa.firstname,' ',pa.middlename,' ',pa.lastname,' (',pa.partycode,')'))",'ct.name','p.name','DATE_FORMAT(av.date, "%d/%m/%Y")');
	...
	function _get_datatables_query(){  
        ...
        $this->readdb->select("av.id,s.sitename,IFNULL(ct.name,'') as sitecity,IFNULL(p.name,'') as siteprovince,av.date,
            sm.partyid,av.createddate,
            CONCAT(pa.firstname,' ',pa.middlename,' ',pa.lastname,' (',pa.partycode,')') as partyname
        ");
        ...
        $this->readdb->join(tbl_party." as pa","pa.id=sm.partyid","INNER");
