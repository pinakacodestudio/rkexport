...
class Supplier_quotation_model extends Common_model {
	...
	{   
        ...
        $query = $this->readdb->select("q.id,q.quotationid as quotationnumber,
                        ...
                        ->join(tbl_party." as buyer","buyer.id=q.partyid","LEFT")
                        ->join(tbl_party." as seller","seller.id=q.sellermemberid","LEFT")
                        ->where($where)
    ...
    function _get_datatables_query(){  
        ...
        $this->readdb->select('q.id,q.quotationdate,q.inquiryno,q.quotationno,q.status,m3.quotationid,c.companyname as cname,u.name as addbyname,
                        ...
                        m.partycode as sellerpartycode,
                        q.sellerpartyid,(select name from '.tbl_party.' where id=q.addedby) as addedby,
                        (select partycode from '.tbl_party.' where id=q.addedby) as addedbycode');
        
        $this->readdb->from($this->_table." as q");
        $this->readdb->join(tbl_party." as m","m.id=q.partyid","left");
        // $this->readdb->join(tbl_party." as m2","m2.id=q.sellerpartyid","left");

        $this->readdb->join(tbl_quotationproducts." as m3","q.id=m3.quotationid","left");
        $this->readdb->join(tbl_company." as c","m.companyid=c.id","left");
        $this->readdb->join(tbl_user." as u","q.addedby=u.id","left");
        $this->readdb->where("q.partyid!=0");
    ...
    function getQuotationDetails($quotationid,$type=''){
        ...
        //q.shippingaddressid
        // q.sellermemberid
        //IFNULL((SELECT channelid FROM ".tbl_member." WHERE id=q.sellermemberid),0)
        //m.gstno
        ...
        //shipper.address
        //shipper.town!='',CONCAT(', ',shipper.town),'')
        ...
        $query = $this->readdb->select("q.id,q.quotationid,q.quotationdate,(SELECT name FROM ".tbl_cashorbank." WHERE id=q.cashorbankid) as bankname,
                                    q.partyid,q.remarks,q.status,q.createddate,
                                ...
                                ")
                            ->from($this->_table." as q")
                            ->join(tbl_party." as m","m.id=q.partyid","LEFT") 
        ...
        $transactiondata['transactiondetail'] = array("id"=>$rowdata['id'],
                                            ...
                                            // "partyname"=>ucwords($rowdata['partyname']),
        ...
        //IFNULL((SELECT CONCAT('[',GROUP_CONCAT(variantvalue),']') FROM ".tbl_quotationvariant." WHERE quotationproductid=o.id),''),' | ',
        //',(SELECT name FROM ".tbl_productcategory." WHERE id=p.categoryid),IFNULL
        //(SELECT CONCAT(' (',name,')') FROM ".tbl_brand." WHERE id=p.brandid),
        //IFNULL((select filename from ".tbl_productimage." where productid=p.id limit 1),'') as productimage
        //o.price,

        $query = $this->readdb->select("o.quantity,o.originalprice,o.tax,o.hsncode,o.discount")
