...
class Memberlogin_model extends Common_model {
	...
	function CheckMemberLogin($emailid,$password) {
	...
	function CheckMemberEmail($username) {
	...
	function resetpassworddata($rcode){
		...
		$this->db->select('id');
	    $this->db->from(tbl_memberemailverification);
		...
		if($query->num_rows()>0){
			...
			$this->db->where($where);
	  		$this->db->delete(tbl_memberemailverification);
			return $result;
		}
	}
	 ...
	 public function insertmemberemailverification($id,$code){
        ...
        if($query->num_rows() > 0){
            ...
            $this->db->update(tbl_memberemailverification);
        }else{
            $data=array('memberid'=>$adminuserid,
                        'rcode'=>$code,
                        'createddate'=>$createddate);
            $insertid = $this->db->insert(tbl_memberemailverification,$data);
        }
    }
