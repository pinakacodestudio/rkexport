...
class Payment_model extends Common_model
{
    ...
    public $column_search = array("vendor.name", 'cb.name', 'cb.accountno', 'pr.transactiondate', 'pr.paymentreceiptno', 'pr.amount', 'IF(pr.isagainstreference=1,"Is Against Invoice","On Account")', "CASE WHEN pr.status=0 THEN 'Pending' WHEN pr.status=1 THEN 'Approve' Else 'Cancel' END"); //set column field database for datatable searchable

    public function __construct()
    {
        parent::__construct();
    }
    // public function getTotalRemainingPaymentAmount($partyid)
    // {
	// 	// 0)) as invoiceamount FROM " . tbl_invoice . " as i WHERE i.partyid=m.id AND i.sellerpartyid=(SELECT mainpartyid FROM " . tbl_membermapping . " WHERE subpartyid=m.id) AND i.status=1), 0) as invoiceamount,

	// 	// IFNULL((SELECT SUM(pr.amount) FROM " . tbl_paymentreceipt . " as pr WHERE pr.sellerpartyid=(SELECT mainpartyid FROM " . tbl_membermapping . " WHERE subpartyid=m.id) AND pr.partyid=m.id AND pr.status=1 AND pr.type=1), 0) as payamount,

	// 	// 						IFNULL((SELECT SUM(cn.amount+cn.taxamount+IFNULL((SELECT SUM(ec.amount) FROM " . tbl_extrachargemapping . " as ec WHERE ec.type=3 AND ec.referenceid=cn.id), 0)-cn.globaldiscount) as creditnoteamount FROM " . tbl_creditnote . " as cn WHERE cn.status=1 AND cn.buyerpartyid=m.id AND cn.sellerpartyid=(SELECT mainpartyid FROM " . tbl_membermapping . " WHERE subpartyid=m.id)), 0) as returnamount
	...
	// 					FROM (
	// 						SELECT
	// 							IFNULL((SELECT SUM(i.amount+i.taxamount+IFNULL((SELECT SUM(ec.amount) FROM " . tbl_extrachargemapping . " as ec WHERE ec.type=2 AND ec.referenceid=i.id), 0)-i.globaldiscount-i.couponcodeamount-IFNULL((SELECT sum(td.redeemamount) FROM " . tbl_transactiondiscount . " as td WHERE td.transactiontype=0 and td.transactionid=i.id),

								

	// 						FROM " . tbl_member . " as m
	// 						WHERE m.id='" . $partyid . "' AND m.status = 1
    ...
    public function getPaymentDetails($id)
    {
        ...
        $query = $this->readdb->select("pr.id,pr.partyid,pr.sellerpartyid,pr.cashorbankid,pr.sellercashorbankid,
        ...
        // ->join(tbl_member." as vendor","vendor.id=pr.sellerpartyid","LEFT")
    ...
    public function updatePurchaseInvoiceStatus($vendorid, $paymentreceiptid, $status)
    {
        ...
        if ($status == 1) {
            $query = $this->readdb->select("i.id")
                ...
                ->where("i.id IN (SELECT invoiceid FROM " . tbl_paymentreceipttransactions . "
								...
								FROM " . tbl_paymentreceipttransactions . " as prt
								WHERE prt.invoiceid = i.id AND prt.paymentreceiptid IN (SELECT id FROM " . tbl_paymentreceipt . " WHERE sellerpartyid=" . $vendorid . " AND partyid=0 AND isagainstreference=1 AND status=1)),0)
    ...
    public function getPaymentDataById($ID)
    {

        $query = $this->readdb->select("id,partyid,cashorbankid,paymentreceiptno,transactiondate,amount,method,remarks,type,status,isagainstreference")
            ->from($this->_table)
            ->where("id", $ID)
            ->get();
    ...
    public function generatePaymentReceiptNo()
    {
    ...
    public function _get_datatables_query()
    {
        ...
        $this->readdb->select("pr.id,pr.partyid,pr.cashorbankid,pr.type,
        ...
        // $this->readdb->join(tbl_member." as vendor","vendor.id=pr.sellerpartyid","LEFT");
        // $this->readdb->join(tbl_cashorbank . " as cb", "cb.id=pr.cashorbankid", "LEFT");
        $this->readdb->where("pr.partyid=0 AND pr.type=1");

        //FILTER VENDOR
        // $this->readdb->where("(pr.sellerpartyid = '".$vendorid."' OR '".$vendorid."'=-1)");
        //FILTER BANK METHOD
