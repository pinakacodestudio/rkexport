...
class Emailformat_model extends Common_model {
    ...
    function getEmailformateListData(){
    ...
    function CheckMailFormatAvailable($mailid,$id='')
    {
        if (isset($id) && $id != '') {
            $query = $this->readdb->query("SELECT id FROM ".tbl_emailtemplate." WHERE mailid ='".$mailid."' AND id <> '".$id."'");
        }else{
            $query = $this->readdb->query("SELECT id FROM ".tbl_emailtemplate." WHERE mailid ='".$mailid."'");
        }
