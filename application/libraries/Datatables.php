...
class Datatables {
	...
	public function join_using($table, $fk, $type = NULL) {
		$this->joins_using[] = array($table, $fk, $type);
		$this->ci->db->join_using($table, $fk, $type);
		return $this;
	}
	...
	// 				} else {
	// 					$this->ci->db->where($this->select[$mColArray[$i]] . ' LIKE', '%' . trim($val) . '%');
	// 				}
	...
	private function get_filtering() {
		...
		for ($i = 0; $i < intval($this->ci->input->post('iColumns')); $i++) {
			if (isset($_POST['sSearch_' . $i]) && $this->ci->input->post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns)) {
				...
				if ($mColArray[$i] == 'eGender') {

					$this->ci->db->where($this->select[$mColArray[$i]] . ' ="' . $this->ci->input->post('sSearch_' . $i) . '"');
				} else if (strpos($this->ci->input->post('sSearch_' . $i), '~') !== false) {

					$miSearch = explode('~', $this->ci->input->post('sSearch_' . $i));

					if (!empty($miSearch[0])) {
						$rangeQuery = $this->select[$mColArray[$i]] . ">=" . $miSearch[0];
					}

					if (!empty($miSearch[1])) {
						$rangeQuery .= (!empty($rangeQuery) ? ' AND ' : '') . $this->select[$mColArray[$i]] . "<=" . $miSearch[1];
					}

					if (!empty($miSearch[0]) || !empty($miSearch[1])) {
						$this->ci->db->where($rangeQuery);
					}
	...
	private function produce_output($output, $charset) {
		...
		foreach ($rResult->result_array() as $row_key => $row_val) {
			$aaData1[$row_key] = ($this->check_mDataprop()) ? $row_val : array_values($row_val);

			foreach ($this->add_columns as $field => $val) {
				if ($this->check_mDataprop()) {
					$aaData[$row_key][$field] = $this->exec_replace($val, $aaData1[$row_key]);
				} else {
					$aaData[$row_key][] = $this->exec_replace($val, $aaData1[$row_key]);
				}
			}
	...
	private function get_total_results($filtering = FALSE) {
		...
		foreach ($this->joins_using as $val) {
			$this->ci->db->join_using($val[0], $val[1], $val[2]);
		}
	...
	private function exec_replace($custom_val, $row_data) {
		...
		if (isset($custom_val['replacement']) && is_array($custom_val['replacement'])) {
			foreach ($custom_val['replacement'] as $key => $val) {
				...
				if ((isset($val)) && $val == "eStatus") {
					$STATUS_BTN = array(
						'content' => $replace_string,
						'class' => 'btn btn-click btn-sm changeStatus ' . (($replace_string == 'Active') ? ' btn-success' : ' btn-warning'),
						'id' => end($row_data),
					);
					$custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), anchor('javascript:;', form_button($STATUS_BTN)), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));
				} else if ((isset($val)) && $val == "vBannerImage") {
					$IMG_URL = "";
					$replace_string = '/' . $replace_string;
					if ($replace_string != "" && file_exists(BANNER_IMAGE_THUMB_PATH . $replace_string)) {
						$IMG_URL = BANNER_IMAGE_THUMB_URL . $replace_string;
					} else {
						$IMG_URL = ADMIN_IMAGE_URL . "no_image.png";
					}
					$custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), img(array("src" => $IMG_URL, 'class' => 'img-circle', "height" => "45px")), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));
				} else {
