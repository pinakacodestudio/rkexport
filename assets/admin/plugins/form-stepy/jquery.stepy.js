...
;(function($) {
  ...
  var methods = {
    init: function(settings) {
      ...
      var self   = this,
          that   = $(this),
          finish = that.children('input[name="submit"]'),
          finish1 = that.children('a[name="saveandprint"]')
      ...
      self.finish1 = (finish1.length === 1) ? finish1 : that.children('.btn .btn-info .stepy-finish');

      if (self.finish1.length) {
        var isForm   = that.is('form'),
            onSubmit = undefined;

        if (isForm && self.opt.finish1) {
          onSubmit = that.attr('onsubmit');

          that.attr('onsubmit', 'return false;');
        }

        self.finish1.on('click.stepy', function(evt) {
          if (self.opt.finish1 && !methods._execute.call(that, self.opt.finish1, self.steps.length - 1)) {
            evt.preventDefault();
          } else if (isForm) {
            if (onSubmit) {
              that.attr('onsubmit', onSubmit);
            ...
            var isSubmit = self.finish1.attr('type') === 'submit';
        ...
        self.steps.last().children('.stepy-navigator').append(self.finish1);
      } else {
        //$.error('Submit button or element with class "stepy-finish" missing!');
      }
