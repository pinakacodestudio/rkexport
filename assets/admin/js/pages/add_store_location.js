...
function resetdata(){
  ...
  $("#link_div").removeClass("has-error is-focused");
...
function checkvalidation(){
  ...
  var isvalidname = isvalidaddress = isvalidcityid = isvalidlatitude = isvalidlongitude = isvalidmobileno= 0 ;
  var isvalidcontactperson = isvalidemail =  isvalidlink = 1;

  PNotify.removeAll();
  if(name == ''){
    $("#name_div").addClass("has-error is-focused");
    new PNotify({title: 'Please enter store name !',styling: 'fontawesome',delay: '3000',type: 'error'});
    isvalidname = 0;
  }else {
    if(name.length<2){
      $("#name_div").addClass("has-error is-focused");
      new PNotify({title: 'Store name require minmum 2 characters !',styling: 'fontawesome',delay: '3000',type: 'error'});
      isvalidname = 0;
    }else{
      isvalidname = 1;
    }
  ...
  if(link != ''){
    if(!isUrlValid(link)){
      $("#link_div").addClass("has-error is-focused");
      new PNotify({title: 'Please enter valid link !',styling: 'fontawesome',delay: '3000',type: 'error'});
      isvalidlink = 0;
    }
  }

  if(isvalidname == 1 && isvalidcontactperson == 1 && isvalidemail == 1 && isvalidmobileno == 1 && isvalidaddress == 1 && isvalidcityid == 1 && isvalidlatitude == 1 && isvalidlongitude == 1 && isvalidlink == 1){

    var formData = new FormData($('#store-locationform')[0]);
    if(ACTION==0){
      var uurl = SITE_URL+"store-location/store-location-add";
      ...
      $.ajax({
        ...
        success: function(response){
          if(response==1){
            new PNotify({title: "Store location  successfully added.",styling: 'fontawesome',delay: '3000',type: 'success'});
              setTimeout(function() { window.location=SITE_URL+"store-location"; }, 1500);
          }else if(response==2){
            new PNotify({title: 'Store location email already exists !',styling: 'fontawesome',delay: '3000',type: 'error'});
            $("#email_div").addClass("has-error is-focused");
          }else if(response==3){
            new PNotify({title: 'Store location mobileno already exists !',styling: 'fontawesome',delay: '3000',type: 'error'});
            $("#mobileno_div").addClass("has-error is-focused");
          }else{
            new PNotify({title: 'Store location  not added !',styling: 'fontawesome',delay: '3000',type: 'error'});
          }
        },
        error: function(xhr) {
      ...
      var uurl = SITE_URL+"store-location/update-store-location";
      ...
      $.ajax({
        ...
        success: function(response){
          if(response==1){
              new PNotify({title: "Store location successfully updated.",styling: 'fontawesome',delay: '1500',type: 'success'});
              setTimeout(function() { window.location=SITE_URL+"store-location"; }, 1500);
          }else if(response==2){
            new PNotify({title: 'Store location email already exists !',styling: 'fontawesome',delay: '3000',type: 'error'});
            $("#email_div").addClass("has-error is-focused");
          }else if(response==3){
            new PNotify({title: 'Store location already exists !',styling: 'fontawesome',delay: '3000',type: 'error'});
            $("#mobileno_div").addClass("has-error is-focused");
          }else{
              new PNotify({title: 'Store location not updated !',styling: 'fontawesome',delay: '3000',type: 'error'});
          }
        },
        error: function(xhr) {
        //alert(xhr.responseText);
