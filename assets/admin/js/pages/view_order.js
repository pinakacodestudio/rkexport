...
function updateorderstatus(){
  ...
  var uurl = SITE_URL+"order/update-order-status";
  ...
  $.ajax({
    ...
    success: function(response){
      ...
      if(response==1){
        new PNotify({title: "Order status successfully updated.",styling: 'fontawesome',delay: '3000',type: 'success'});
        setTimeout(function() { window.location=SITE_URL+"order"; }, 1500);
        
      }else{
        new PNotify({title: "Order status not updated !",styling: 'fontawesome',delay: '3000',type: 'error'});
      }
    },
    error: function(xhr) {
    //alert(xhr.responseText);
...
function changeinstallmentstatus(status, installmentid){
    var uurl = SITE_URL+"order/update-installment-status";
        if(installmentid!=''){
...
function printorderinvoice(id,withoutprice=0){
