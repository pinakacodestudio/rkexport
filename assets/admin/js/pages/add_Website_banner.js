...
function checkvalidation() {
  ...
  var isvalidtitle = isvalidfiletext = isvalidaltalttext = isvaliddescription= 0;
  ...
  if(filetype==1 ){
    if(alttext.trim() == ''){
      $("#alttext_div").addClass("has-error is-focused");
      new PNotify({title: 'Please enter alt text !',styling: 'fontawesome',delay: '3000',type: 'error'});
      isvalidaltalttext = 0;
    }else { 
      if(alttext.length<3){
        $("#title_div").addClass("has-error is-focused");
    ...
    if(ACTION == 0){    
      ...
      $.ajax({
        ...
        success: function(response){
          if(response==1){
            new PNotify({title: "Website banner successfully added.",styling: 'fontawesome',delay: '3000',type: 'success'});
            //resetdata();
            setTimeout(function() { window.location=SITE_URL+"website_banner"; }, 1500);
          }else if(response==2){
            new PNotify({title: 'Website banner file not uploaded !',styling: 'fontawesome',delay: '3000',type: 'error'});
          }else if(response==3){
            new PNotify({title: 'Invalid type of Website banner file !',styling: 'fontawesome',delay: '3000',type: 'error'});
          }else{
            new PNotify({title: "Website banner not added !",styling: 'fontawesome',delay: '3000',type: 'error'});
          }
        },
        error: function(xhr) {
      ...
      var uurl = SITE_URL+"website-banner/update_website_banner";
      $.ajax({
        url: uurl,
        type: 'POST',
        data: formData,
        ...
        success: function(response){
          if(response==1){
            new PNotify({title: "Website banner successfully updated.",styling: 'fontawesome',delay: '3000',type: 'success'});
            setTimeout(function() { window.location=SITE_URL+"website_banner"; }, 1500);
          }else if(response==2){
            new PNotify({title: 'Website banner file not uploaded !',styling: 'fontawesome',delay: '3000',type: 'error'});
          }else if(response==3){
            ...
            new PNotify({title: "Website banner not updated !",styling: 'fontawesome',delay: '3000',type: 'error'});
          }
        },
        error: function(xhr) {
        //alert(xhr.responseText);
