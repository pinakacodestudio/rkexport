...
function getproductwisesalesdata(){

  var uurl = SITE_URL+"product-wise-sales-report/getproductwisesalesdata";
  var formData = new FormData($('#productwisesalesform')[0]);
  $.ajax({
    url: uurl,
    type: 'POST',
    data: formData,
    ...
    success: function(response){
      ...
      if ( $.fn.DataTable.isDataTable('#productwisesalestable') ) {
        $('#productwisesalestable').DataTable().destroy();
      }
      $('#productwisesalestable').empty();
      $('#productwisesalestable').html(response);
      
      // if(!$.isEmptyObject(dataObject.DATA) && dataObject.DATA!=undefined){
        var fixedColumns = 3;
        table =   $('#productwisesalestable').DataTable({
         ...
         /*  "scrollCollapse": true,
          ...
          fnFooterCallback: function(row, data, start, end, display) {
            ...
            if(!$.isEmptyObject(data)){
              $.footer = '';
              $('.footercls').remove();
              if($('#productwisesalestable tfoot').length==0){
                $.footer += '<tfoot>';
              }
              
              $.footer += '<tr class="footercls">';
              $.footer += '<th colspan="3" class="totalrows text-right">Total Sales</th>';
              ...
              if($('#productwisesalestable tfoot').length==0){
                $.footer += '</tfoot>';
                $('#productwisesalestable tbody').after($.footer);
              }else{
                $('#productwisesalestable tfoot').prepend($.footer);
              }
            }else{
              $('#productwisesalestable tfoot').remove();
            }
          }
        });
...
function exportproductwisesalesreport(){
  ...
  var totalRecords =$("#productwisesalestable").DataTable().page.info().recordsDisplay;
  $.skylo('end');
  if(totalRecords != 0){
    window.location= SITE_URL+"product-wise-sales-report/exportproductwisesalesreport?startdate="+startdate+"&enddate="+enddate+"&channelid="+channelid+"&memberid="+memberid+"&categoryid="+categoryid+"&productid="+productid+"&status="+status+"&datetype="+datetype;
  }else{
...
function exporttopdfproductwisesalesreport(){
  ...
  $.skylo('end');
  if(totalRecords != 0){
    window.location= SITE_URL+"product-wise-sales-report/exporttopdfproductwisesalesreport?startdate="+startdate+"&enddate="+enddate+"&channelid="+channelid+"&memberid="+memberid+"&categoryid="+categoryid+"&productid="+productid+"&status="+status+"&datetype="+datetype;
  }else{
    new PNotify({title: 'No data available !',styling: 'fontawesome',delay: '3000',type: 'error'});
  }
...
function printproductwisesalesreport(){
  ...
  var totalRecords =$("#productwisesalestable").DataTable().page.info().recordsDisplay;
    $.skylo('end');
    if(totalRecords != 0){
      var uurl = SITE_URL + "product-wise-sales-report/printproductwisesalesreport";
      $.ajax({
        url: uurl, 
        type: 'POST',
