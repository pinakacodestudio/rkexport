...
function checkvalidation(){
    ...
    if(isvalidsubject==1 &&  isvalidmessage==1){
        ...
        var formData = new FormData($('#painpointreplyform')[0]);
        
        var uurl = SITE_URL+"Painpointreply/addpainpointreply";
        $.ajax({
          url: uurl,
          type: 'POST',
          data: formData,
          ...
          success: function(response){
            if(response==1){
              new PNotify({title: "Painpoint reply successfully sended.",styling: 'fontawesome',delay: '3000',type: 'success'});
              setTimeout(function() { window.location=SITE_URL+"painpointreply"; }, 1500);
            }else{
              new PNotify({title: "Painpoint reply not sended !",styling: 'fontawesome',delay: '3000',type: 'error'});
            }
          },
          error: function(xhr) {
