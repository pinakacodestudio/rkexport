...
function viewdescription(id){
  ...
  var uurl = SITE_URL+"offer/viewofferdescription";
  $.ajax({
    url: uurl,
    type: 'POST',
    data: {id:String(id)},
...
function viewmemberlist(id,channelid){
  ...
  var uurl = SITE_URL+"offer/viewmemberlist";
  $.ajax({
    url: uurl,
    type: 'POST',
    ...
    success: function(response){
      ...
      if(JSONObject.length > 0){
        ...
        for(i=0;i<JSONObject.length;i++) {
            ...
            html += "<td>"+JSONObject[i]['membercode']+"</td>";
            html += "</tr>";
      ...
      // $('#pagetitle').html(JSONObject['pagetitle']);
      //$('#memberdata').html(JSONObject['description'].replace(/&nbsp;/g, ' '));
    },
    error: function(xhr) {
    //alert(xhr.responseText);
    },
