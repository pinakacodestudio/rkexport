function resetdata(){
  ...
  $('#mainmenu,#rightsid').selectpicker('refresh');
}
function checkvalidation(action){
  ...
  if(isvalidname == 1 && isvalidmainmenu == 1 && isvalidmenuurl == 1){
    ...
    if(ACTION==0){
      var uurl = SITE_URL+"menu/add-sub-menu";
      ...
      $.ajax({
        ...
        success: function(response){
          if(response==1){
            new PNotify({title: "Sub Menu successfully added.",styling: 'fontawesome',delay: '3000',type: 'success'});
            resetdata();
        }else if(response==2){
            new PNotify({title: 'Sub Menu already exists !',styling: 'fontawesome',delay: '3000',type: 'error'});
        }else{
          new PNotify({title: 'Sub Menu not added !',styling: 'fontawesome',delay: '3000',type: 'error'});
        }
        },
        error: function(xhr) {
      ...
      var uurl = SITE_URL+"menu/update-sub-menu";
      ...
      $.ajax({
        ...
        success: function(response){
          if(response==1){
            new PNotify({title: "Sub Menu successfully updated.",styling: 'fontawesome',delay: '1500',type: 'success'});
            setTimeout(function() { window.location=SITE_URL+"menu/sub-menu"; }, 1500);
        }else if(response==2){
            new PNotify({title: 'Sub Menu already exists !',styling: 'fontawesome',delay: '3000',type: 'error'});
        }else{
            new PNotify({title: 'Sub Menu not updated !',styling: 'fontawesome',delay: '3000',type: 'error'});
        }
        },
        error: function(xhr) {
        //alert(xhr.responseText);
