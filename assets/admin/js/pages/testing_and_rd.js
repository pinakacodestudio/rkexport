...
$(document).ready(function() {
  ...
  oTable = $('#testingtable').DataTable({
      ...
      "ajax": {
        "url": SITE_URL+"testing-and-rd/listing",
        "type": "POST",
        "data": function ( data ) {
          data.processid= $("#processid").val();
          data.batchid = $("#batchid").val();
          // alert(data.batchid);
          data.testedid = $("#testedid").val();
...
var totalRecords =$("#testingtable").DataTable().page.info().recordsDisplay;
$.skylo('end');
if(totalRecords != 0){
...
function changestatus(status, testingId){
  var uurl = SITE_URL+"testing-and-rd/update-status";
      if(testingId!=''){
              ...
              function(isConfirm){   
                if (isConfirm) {  
                  $.ajax({
                      url: uurl,
                      type: 'POST',
                      data: {status:status,testingId:testingId},
                      beforeSend: function(){
                        $('.mask').show();
                        $('#loader').show();
                      },
