...
function loadproductdata() {
    ...
    if(productcategory!=0){
      var uurl = SITE_URL+"vendor/getProductByCategorywithNotAssignVendor";
      $.ajax({
        url: uurl,
        type: 'POST',
        data: {categoryid:productcategory,vendorid:vendorid},
        dataType: 'json',
        async: false,
                ...
                producttablehtml += '<td>\
                                      ...
                                      <div class="col-md-3 pr-xs pl-n">\
                                        <div class="form-group m-n" id="memberstock'+(i+1)+'_div">\
                                          <input type="text" name="memberstock[]" id="memberstock'+(i+1)+'" class="form-control text-right m-n" value="" onkeypress="return isNumber(event)" placeholder="Vendor Stock">\
                                        </div>\
                                      </div>\
                                      <div class="col-md-3 pr-n pl-xs">\
                                        <div class="form-group m-n" for="channelid" id="channelid'+(i+1)+'_div">\
                                          <select class="form-control selectpicker mt-n" id="channelid'+(i+1)+'" name="channelid[]" data-live-search="true" data-select-on-tab="true" data-size="5">\
                                            '+ChannelDataHTML+'\
                                          </select>\
                                        </div>\
                                      </div>\
...
function checkvalidation(type='add') {
  ...
  if(isvalidcategoryid==1 && isvalidproduct==1 && isvalidmultipleprice==1){
    ...
    var formData = new FormData($('#vendorproductform')[0]);
    if(ACTION == 0){    
      var uurl = SITE_URL+"vendor/vendor-product-add";
      $.ajax({
        url: uurl,
        type: 'POST',
        data: formData,
        ...
        success: function(response){
          if(response==1){
            new PNotify({title: "Vendor product successfully added!",styling: 'fontawesome',delay: '3000',type: 'success'});
            // resetdata();
            if(type=='addandnew'){
              setTimeout(function() { location.reload(); }, 1500);
            }else{
              setTimeout(function() { window.location=SITE_URL+"vendor/vendor-detail/"+$("#vendorid").val()+"/products"; }, 1500);
            }

          }else{
            new PNotify({title: "Vendor product not added !",styling: 'fontawesome',delay: '3000',type: 'error'});
          }
        },
        error: function(xhr) {
        //alert(xhr.responseText);
      ...
      var uurl = SITE_URL+"vendor/vendor-product-edit";
      $.ajax({
        url: uurl,
        type: 'POST',
        data: formData,
        ...
        success: function(response){
          if(response==1){
            new PNotify({title: "Vendor product successfully updated !",styling: 'fontawesome',delay: '3000',type: 'success'});
            setTimeout(function() { window.location=SITE_URL+"vendor/vendor-detail/"+$("#vendorid").val()+"/products"; }, 1500);
          }else{
            new PNotify({title: "Vendor product not updated !",styling: 'fontawesome',delay: '3000',type: 'error'});
          }
        },
        error: function(xhr) {
        //alert(xhr.responseText);
...
/* function searchmembercode(){
    ...
    if(sellercode == ""){
        ...
        new PNotify({title: 'Please enter seller member code !',styling: 'fontawesome',delay: '3000',type: 'error'});
        isvalidsellercode = 0;
    }else{
        if(sellercode.length != 8){
            $("#sellercode_div").addClass("has-error is-focused");
            new PNotify({title: 'Seller member code required between 8 characters !',styling: 'fontawesome',delay: '3000',type: 'error'});
            isvalidsellercode = 0;
        }
    }
